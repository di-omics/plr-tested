...
of the whole-genome sequencing preparation workflow, expressed as PyLabRobot
...
seconds assumption is now confirmed, so the authorized WGS/WGA programs are believed correct,
...
1. **A cycling Method will not run without a PreMethod pre-warm first.** `ExecuteMethod`
   ...
   pre-warm-then-run pattern the authorized WGS/WGA workflow source specifies for the
   WGA program ("start the
...
from an authorized WGS/WGA workflow source (RUO). The targeted PCR programs come from the
...
- (*) **Lid temperature.** The targeted PCR protocol does not state one. 105 C is the standard
  heated-lid temperature for Q5 PCR and matches the authorized WGS/WGA workflow's
  library-amplification program, which is also a Q5
...
Two translations were needed to map the authorized WGS/WGA workflow onto this instrument,
and both are worth
knowing:
