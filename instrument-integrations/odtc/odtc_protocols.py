"""
odtc_protocols.py - the thermal programs of the ResolveDNA Whole Genome Single-Cell
...
Primary source
--------------
  BioSkryb Genomics, "ResolveDNA Whole Genome Single-Cell Core Kit, 96 Reactions",
  document TAS-068.5, revision 05/2025.
...
# The block starts a method from wherever it happens to be. TAS-068.5 never specifies
# a start block temperature, so use the one PLR's ODTC notebook uses: room temperature.
START_BLOCK_C_DEFAULT = 25.0

# TAS-068.5 lid temperatures, per table.
...
# ===========================================================================
# Amplicon-seq library prep. A DIFFERENT protocol from the ResolveDNA TAS-068.5
# programs above.
#
# Source: "Amplicon-seq Library Prep" protocol, di-omics internal, updated
...
#     heated-lid temperature for Q5 PCR and matches the TAS-068.5 LIB-AMP program,
...
def ampseq_pcr1(anneal_c: float = AMPSEQ_ANNEAL_C, num_cycles: int = 30) -> "Protocol":
    """Amplicon-seq PCR1 (target amplification).

    Source: Amplicon-seq Library Prep, PCR1 cycling table.
      98 C 30 s            x1
...
def ampseq_pcr2(anneal_c: float = AMPSEQ_ANNEAL_C, num_cycles: int = 8) -> "Protocol":
    """Amplicon-seq PCR2 (Nextera indexing).

    Source: Amplicon-seq Library Prep, PCR2 cycling table.
      98 C 30 s            x1
...
# ampseq primer volumes live in the liquid-handling protocol and not in this file.
#
