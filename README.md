...
- [`instrument-integrations/`](instrument-integrations) - the instruments the
  ...
  Cycler) over SiLA/SOAP: the ResolveDNA thermal programs as PyLabRobot protocols,
...
- [`packages/`](packages) - self-contained, QC-gated assay products built on the
  ...
  - [`gene-edit/`](packages/gene-edit) - QC-gated PTA + amplicon-seq for confirming
    gene edits.
...
| Full lidded ampseq choreography with the ODTC **called live** at both thermal legs: 13 motion legs, 22 SUCCESS, 0 failures, deck self-returned | passed on the instrument |
