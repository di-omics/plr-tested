...
VOL_WASH_ADD = required_positive("methylation_seq.cleanup.wash_add_ul")
VOL_WASH_REMOVE = required_positive("methylation_seq.cleanup.wash_remove_ul")
VOL_RESIDUAL_WASH_REMOVE = required_positive("methylation_seq.cleanup.residual_remove_ul")
...
async def assign_deck(lh: LiquidHandler, cleanup: Cleanup) -> Dict[str, object]:
    ...
    print("\nDeck:")
    print("  rail48 pos1 = p50 tips (elution and residual-wash motions)")
    print("  rail48 pos2 = p300 tips (binding, supernatant, and wash motions)")
    print("  rail35 pos2 = magnet + CellTreat_96_wellplate_350ul_Fb work plate, column 1")
    print("  rail35 pos3 = 12-well reservoir/trough")

    print("\nReservoir map:")
    print(f"  {TROUGH_BINDING} = operator-specified binding suspension")
    print(f"  {TROUGH_WASH1} = operator-specified wash solution 1")
    print(f"  {TROUGH_WASH2} = operator-specified wash solution 2")
    print(f"  {TROUGH_ELUTION} = operator-specified elution solution")
...
LEG_ORDER = [
    "binding-add", "supernatant-remove",
    "wash-add1", "wash-remove1", "wash-add2", "wash-remove2",
...
async def run_all(lh: LiquidHandler, r: Dict[str, object], cleanup: Cleanup, discard_tips: bool):
    ...
    print("Use the operator-approved incubation and magnetic-capture handoffs; timings are")
    print("not modeled here.")
...
async def main():
    parser = argparse.ArgumentParser(
        description="methylation-sequencing workflow magnetic cleanup, column 1, mag pos2 + trough pos3."
    )
    ...
    parser.add_argument("--return-tips", action="store_true",
                        help="Return tips instead of discarding. Dry observation only; real cleanup runs "
                             "must discard tips by default.")
    ...
    # Discard by default because returning cleanup tips is a carryover hazard.
    # --return-tips is for dry observation only.
