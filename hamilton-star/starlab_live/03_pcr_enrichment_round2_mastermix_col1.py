...
# PCR-enrichment operator-defined stage-2 addition, column 1
#
# Purpose:
...
# - Wet-method composition, sample assignment, transfer volume, and thermal settings are
#   required local parameters and intentionally absent from this public file.
...
#
# PLR_METHOD_PARAMETERS_FILE must provide the transfer value and approved
# thermal-program identifier before hardware setup.
...
PCR2_MM_STEP = Step(
    ...
    manual_stop=(
        f"Follow the approved local thermal handoff {THERMAL_PROGRAM_ID!r}."
    ),
)
...
async def assign_deck(lh: LiquidHandler) -> Dict[str, object]:
    ...
    print("  rail48 pos1 = p50 tips, used for operator-defined stage 2")
    ...
    print("\nPCR-enrichment stage-2 mode:")
    print("  destination and source col 1 A-H are prepared by the operator")
    print("  destination col 1 A-H includes operator-approved i5 + i7 inputs")
    print(f"  destination input = {DESTINATION_INPUT_VOLUME_UL} uL (operator profile)")
...
async def transfer_pcr2_master_mix(lh: LiquidHandler, r: Dict[str, object], discard_tips: bool, tip_col: int):
    ...
    try:
        print(f"Aspirating {step.volume_ul} uL x8 from stage-2 source col {SOURCE_COL}...")
        await lh.aspirate(
...
async def main():
    parser = argparse.ArgumentParser(
        description="PCR-enrichment operator-defined stage-2 addition, column 1."
    )
