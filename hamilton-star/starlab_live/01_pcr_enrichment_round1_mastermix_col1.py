...
# PCR-enrichment operator-defined stage-1 addition, column 1
#
# Purpose:
...
# - Wet-method composition, input volume, transfer volume, and thermal settings are
...
async def assign_deck(lh: LiquidHandler) -> Dict[str, object]:
    ...
    print("  rail48 pos1 = p50 tips, used for operator-defined stage 1")
...
async def transfer_pcr1_master_mix(lh: LiquidHandler, r: Dict[str, object], discard_tips: bool, tip_col: int):
    ...
    try:
        print(f"Aspirating {step.volume_ul} uL x8 from stage-1 source col {SOURCE_COL}...")
        await lh.aspirate(
...
async def main():
    parser = argparse.ArgumentParser(
        description="PCR-enrichment operator-defined stage-1 addition, column 1."
    )
