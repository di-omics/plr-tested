# WGS preparation -> PCR enrichment full dry deck setup checklist
...
| 35 | Rail 35 · p1 (second slot) | CellTreat 229195/229196 96-well source plate | Empty for dry work; WGS preparation addresses columns 1 and 3 and PCR enrichment addresses column 1 |
...
1. Print the combined inert plan:
   ...
   ```bash
   python starlab_live/run_wgs_prep_pcr_enrichment_LIDDED_1col_full_dry.py \
...
2. Run both connection-free deck/model previews on the Pi:
   ...
   ```bash
   ./run_on_pi.sh starlab_live/run_wgs_prep_pcr_enrichment_LIDDED_1col_full_dry.py \
...
5. Open a durable Pi-local terminal, then run the physical command from inside
   ...
   ```bash
   ssh -t starpi
   tmux new-session -A -s wgs_prep_pcr_enrichment_full_dry
   source ~/star-lab/env/bin/activate
   cd ~/plr-tested-run
   python -u starlab_live/run_wgs_prep_pcr_enrichment_LIDDED_1col_full_dry.py \
     --mode star --sample-count 8 \
     --confirm RUN_WGS_PREP_PCR_ENRICHMENT_LIDDED_1COL_FULL_DRY \
   ...
   `tmux attach -t wgs_prep_pcr_enrichment_full_dry`. The operator must remain physically at
   the E-stop even while the terminal is disconnected.

6. After WGS preparation exits 0, the combined runner stops at a mandatory physical
   handoff. Inspect the deck before typing the displayed token. PCR enrichment cannot
   spawn unless the exact fresh observation is entered:

   ```text
   WGS_PREP_FINAL_PLATE_R35P0_LID_R35P4_HHS_EMPTY_TIPS_CLEAR_ISWAP_PARKED
   ```

   At this hold, verify plate rail35 pos0, lid rail35 pos4, HHS empty, returned
   tips/channels clear, iSWAP parked, magnet and ODTC empty/open, and all PCR enrichment
...
The combined runner validates dry WGS preparation and PCR enrichment
choreography. A future wet implementation must load a locally approved,
operator-supplied method profile rather than embedding a biological recipe.
That profile must define all sample and reagent volumes, mixing and incubation
settings, thermal programs, cleanup parameters, and tip-disposition rules.
...
control validation, and a PCR enrichment SOP supplied by the team.
