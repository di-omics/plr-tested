...
TRANSFER_VOL_UL = required_positive("pcr_enrichment.round_1_transfer_ul")
...
async def main():
    parser = argparse.ArgumentParser(
        description="PCR enrichment round 1 operator-volume transfer using preserved BAD tip geometry."
    )
    ...
    try:
        ...
        print("")
        print("PCR ENRICHMENT ROUND 1 OPERATOR-VOLUME FIRST-COLUMN LIQUID TRANSFER")
        print("")
        print("Deck:")
        print("  rail48 pos1 = p50 tips")
        print("  rail35 pos1 = source plate, col1 A-H = operator-prepared round 1 reagent")
        ...
        if args.confirm != "RUN_PCR_ENRICHMENT_ROUND1_OPERATOR_VOLUME":
            raise RuntimeError("Refusing to run. Add: --confirm RUN_PCR_ENRICHMENT_ROUND1_OPERATOR_VOLUME")

        print("Picking up p50 tip column 1...")
        await lh.pick_up_tips(tip_col)

        print(f"Aspirating {TRANSFER_VOL_UL} uL x8 from source rail35 pos1 col1...")
        await lh.aspirate(
            source_col,
        ...
        print(
            f"Dispensing {TRANSFER_VOL_UL} uL x8 to destination rail35 pos0 col1 "
            f"with {P50_BLOWOUT_AIR_VOLUME} uL blowout..."
        )
        await lh.dispense(
            dest_col,
            vols=[TRANSFER_VOL_UL] * 8,
        ...
        print("SUCCESS: PCR enrichment round 1 operator-volume liquid transfer completed.")
