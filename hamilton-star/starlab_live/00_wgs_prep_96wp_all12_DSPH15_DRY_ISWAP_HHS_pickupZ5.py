...
# whole-genome sequencing Validation - WGS prep beginning
#
# FULL-PLATE + PICKUP-Z5 VARIANT 2026-07-09 (derived from
# 00_wgs_prep_96wp_demo_all12_DSPH15_DRY_ISWAP_R27P2_HHS.py):
# - Same full-plate dry demo: lysis then reaction, source col 1 -> destination cols 1-12.
# - Adds --iswap-pickup-z-offset-mm (default 5.0) so the iSWAP grabs the work plate 5 mm higher
#   off rail35 pos0. The parent picked up at the plate's natural location, which grabbed too low.
# - iSWAP drop offsets now default to the validated x12.0 / y54.5 / z17.0.
# - Return leg is the separate validated test_iswap_plate_rail27pos2_hhs_to_rail35pos0_return.py.
# - All pipetting geometry (heights, XY offsets, 7.0 uL blowout) is UNCHANGED from the parent.
# - DRY ONLY: full-demo-iswap requires --return-tips.
...
# - lysis adds the operator-approved stage 1 volume using p10 tip column 1.
# - reaction adds the operator-approved stage 2 volume using p10 tip column 2.
...
# - Water test worked with destination wells prefilled to the operator-approved input volume.
# - Small-volume modes use p10; LIBRARY_PCR uses p50.
# - Source aspiration height = 0.0.
# - Work/destination dispense height = 0.5.
# - Work/destination dispense XY = Coordinate(-0.68, 3.22, 0.0).
# - P10 blowout = 7.0 uL with 1.0 sec post-dispense settle.
# - Default whole-genome sequencing preparation map: lysis p10 col1, reaction p10 col2.
#
...
# - Default DNA_FRAGMENTATION tip column changed to p10 column 3 because column 2 was picked/discarded during the failed test.
#
# P10 WATER-RETENTION TEST PATCH:
# - Keeps p10 for DNA_FRAGMENTATION and small-volume modes.
# - DNA_FRAGMENTATION default tip column set to p10 column 2 for this water test.
...
#
# P10 LOW-HEIGHT DNA_FRAGMENTATION TEST PATCH:
# - Keeps p10 for small-volume modes; p50 remains for LIBRARY_PCR.
# - P10 source aspiration height = 0.0.
# - P10 work/destination dispense height = 0.5.
# - P10 blowout = 5.0 uL, with 1.0 sec post-dispense settle before tip discard/return.
# - Default DNA_FRAGMENTATION tip column changed to p10 column 8 for fresh upper-column testing.
...
# - Tip plan unchanged: p10 for small-volume stages, p50 for library PCR.
#
...
# - Keep the validated tip plan: p10 for small-volume stages, p50 for library PCR.
...
#     dna_fragmentation = operator-approved stage 3 reagent and volume, p10
#     end_repair   = operator-approved stage 4 reagent and volume, p10
#     adapter = operator-approved stage 5 reagent and volume, p10
#     ligation_mix = operator-approved stage 6 reagent and volume, p10
#     library_pcr = operator-approved stage 7 reagent and volume, p50
#
...
# P50 for LIBRARY_PCR uses same x/y and height as the validated column-1 geometry.
...
STEPS = {
    "lysis": Step(
        ...
        VOL_LYSIS,
        "p10",
        "Load source rail35 pos1 column 1 with operator-approved WGS stage 1 reagent. Destination contains operator-prepared input.",
        "Seal/spin, then follow the operator-approved WGS stage 1 handoff.",
    ),
    "reaction": Step(
        "reaction",
        "Reaction Mix",
        VOL_REACTION,
        "p10",
        "Load source rail35 pos1 column 1 with operator-approved WGS stage 2 reagent. Destination contains operator-approved stage 1 output.",
        "Seal/spin, then follow the operator-approved WGS stage 2 handoff.",
    ),
}
...
async def assign_deck(lh: LiquidHandler) -> Dict[str, object]:
    print("Assigning Validation WGS prep deck: SWAP-SOURCE column-1 DISCARD-TIP version...")
    ...
    print("\nSwap-source map:")
    print("  dna_fragmentation: source col 1 = operator-approved stage 3 reagent and volume")
    print("  end_repair: source col 1 = operator-approved stage 4 reagent and volume")
    print("  adapter: source col 1 = operator-approved stage 5 reagent and volume")
    print("  ligation_mix: source col 1 = operator-approved stage 6 reagent and volume")
    print("  library_pcr: source col 1 = operator-approved stage 7 reagent and volume")
    print("\nP10 geometry:")
...
async def main():
    parser = argparse.ArgumentParser(description="WGS prep full-plate dry demo: source column 1 to destination columns 1-12 with 1.5 mm destination dispense height.")
    ...
    try:
        ...
        if args.mode == "full-demo-iswap":
            if args.confirm != "RUN_FULL_WGS_PREP_ISWAP_DEMO":
                raise RuntimeError("Refusing to run full demo. Add: --confirm RUN_FULL_WGS_PREP_ISWAP_DEMO")
            if discard_tips:
            ...
            print(f"  3. iSWAP work plate to HHS rail27 pos2 (pickup +{args.iswap_pickup_z_offset_mm} mm)")
