"""Full whole-genome amplification/HHS iSWAP + targeted PCR dry choreography runner (Bio Validation 0).
...
Phases:
  1. whole-genome amplification/HHS iSWAP forward move: rail35 pos0 -> rail27 pos2 (HHS)
  2. whole-genome amplification/HHS iSWAP return move:  rail27 pos2 (HHS) -> rail35 pos0
  3. targeted PCR full built dry choreography (09_ampseq_full_built_end_to_end_dry.py)

Refuses to run without: --confirm RUN_FULL_PTA_AMPSEQ_DRY_RETURN_TIPS
...
AMPSEQ_SCRIPT = AMPSEQ_DIR / "09_ampseq_full_built_end_to_end_dry.py"
...
def preflight():
    missing = [p for p in (FORWARD_SCRIPT, RETURN_SCRIPT, AMPSEQ_SCRIPT) if not p.exists()]
    if missing:
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Full PTA/HHS iSWAP forward + return, then ampseq full dry "
    ...
    print("")
    print("FULL PTA/HHS iSWAP -> ampseq DRY CHOREOGRAPHY (return tips)")
    ...
    run_phase(3, "ampseq full built dry choreography", AMPSEQ_CMD)
