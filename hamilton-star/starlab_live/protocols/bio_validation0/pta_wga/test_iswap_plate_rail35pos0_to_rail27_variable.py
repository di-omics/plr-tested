...
async def main():
    parser = argparse.ArgumentParser(
        description="iSWAP isolated plate move test: rail35 pos0 -> rail27 variable pos with adjustable offsets."
    )
    parser.add_argument("--mode", choices=["deck", "move"], default="deck")
    parser.add_argument("--pickup-z-offset-mm", type=float, default=8.5)
    parser.add_argument("--drop-position", type=int, default=2)
    parser.add_argument("--drop-x-offset-mm", type=float, default=14.0)
    parser.add_argument("--drop-y-offset-mm", type=float, default=31.5)
    parser.add_argument("--drop-z-offset-mm", type=float, default=0.0)
    parser.add_argument("--confirm", default="")
    args = parser.parse_args()

    if args.drop_position < 0 or args.drop_position > 4:
        raise ValueError("--drop-position must be 0..4 for PLT_CAR_L5AC_A00")
    ...
    try:
        ...
        if args.confirm != "RUN_ISWAP_PLATE_TEST":
            raise RuntimeError("Refusing to move. Add: --confirm RUN_ISWAP_PLATE_TEST")

        print("MOVING PLATE with iSWAP...")
        print("Use EMPTY sacrificial plate. Destination/nest must be physically empty. Hand near E-stop.")
