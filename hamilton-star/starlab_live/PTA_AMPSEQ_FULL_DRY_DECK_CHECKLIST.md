# whole-genome amplification -> Targeted PCR full dry deck setup checklist
...
1. Print the combined inert plan:
   ...
   ```bash
   python starlab_live/run_pta_ampseq_LIDDED_1col_full_dry.py \
...
2. Run both connection-free deck/model previews on the Pi:
   ...
   ```bash
   ./run_on_pi.sh starlab_live/run_pta_ampseq_LIDDED_1col_full_dry.py \
...
5. Open a durable Pi-local terminal, then run the physical command from inside
   ...
   ```bash
   ssh -t starpi
   tmux new-session -A -s pta_ampseq_full_dry
   source ~/star-lab/env/bin/activate
   cd ~/plr-tested-run
   python -u starlab_live/run_pta_ampseq_LIDDED_1col_full_dry.py \
     --mode star --sample-count 8 \
     --confirm RUN_PTA_AMPSEQ_LIDDED_1COL_FULL_DRY \
   ...
   `tmux attach -t pta_ampseq_full_dry`. The operator must remain physically at
   the E-stop even while the terminal is disconnected.

6. After whole-genome amplification exits 0, the combined runner stops at a mandatory physical
...
The supplied the vendor guide is `the kit user guide | 05/2025`, whole-genome
Single-Cell Core Kit, 96-well format. It defines whole-genome amplification and Universal
Library Preparation for downstream WGS. It does not define the di-omics
targeted Targeted PCR workflow. Page 14 also states that a separate protocol is
...
control validation, and a targeted Targeted PCR SOP supplied by the team.
