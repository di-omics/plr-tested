...
def main():
    parser = argparse.ArgumentParser(
        description="Targeted PCR col1 full choreography with ODTC thermocycler handoffs, dry (tips returned)."
    )
    parser.add_argument(
        "--confirm",
        default="",
        help="Required to run the choreography on hardware: --confirm RUN_TARGETED_PCR_ODTC_FULL",
    )
    args = parser.parse_args()

    print("")
    print("FULL TARGETED PCR + ODTC CHOREOGRAPHY, COLUMN 1, DRY (tips returned)")
    ...
    if args.confirm != "RUN_TARGETED_PCR_ODTC_FULL":
        print("")
        print("Refusing to run. This moves the arm through nine transfers, including two ODTC")
        print("round trips and the magnet round trip. Add: --confirm RUN_TARGETED_PCR_ODTC_FULL")
