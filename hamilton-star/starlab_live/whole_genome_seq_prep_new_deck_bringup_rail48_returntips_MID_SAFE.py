...
# -----------------------------------------------------------------------------
# whole-genome sequencing WGS preparation - NEW DECK BRINGUP / BASIC MOVEMENTS
...
# -----------------------------
# whole-genome sequencing preparation source layout / volumes
# -----------------------------
...
async def run_whole_genome_seq_addition_scaffold(lh: LiquidHandler, r: Dict[str, object]):
    ...
    print("\n=== whole-genome sequencing preparation ADDITION SCAFFOLD: small reagent additions with p10 ===")
    await lh.pick_up_tips(r["p10_tips_r48"]["A1:H1"])
    try:
        small_reagents = [
            (SRC_LYSIS_COL, VOL_LYSIS, "lysis reagent mix"),
            (SRC_REACTION_COL, VOL_REACTION, "WGS preparation reaction mix"),
    ...
    print("\n=== whole-genome sequencing preparation ADDITION SCAFFOLD: operator-profile library amplification volume with p50 ===")
    ...
    print("\n=== whole-genome sequencing preparation CLEANUP ADDITION SCAFFOLD: trough -> rail35 pos1 mag 96WP placeholder ===")
    await lh.pick_up_tips(r["p300_tips_r48"]["A1:H1"])
    try:
        cleanup_additions = [
            (TROUGH_BEADS, VOL_BEADS, "SPRI beads"),
            (TROUGH_ETOH1, VOL_ETOH, "operator-approved wash 1"),
        ]
        if WASH_COUNT == 2:
            cleanup_additions.append(
                (TROUGH_ETOH2, VOL_ETOH, "operator-approved wash 2")
            )
        cleanup_additions.append((TROUGH_ELUTION, VOL_ELUTION, "Elution Buffer"))
        for dest_col in DEST_COLUMNS:
    ...
    print("SUCCESS: whole-genome sequencing preparation new-deck addition scaffold completed.")
    print("NOTE: no cleanup removals, no residual ethanol removal, no final eluate transfer in this file.")


async def main():
    parser = argparse.ArgumentParser(description="STAR new-deck bringup for whole-genome sequencing WGS preparation automation")
    parser.add_argument(
        "--mode",
