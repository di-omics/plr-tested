...
#
# SINGLE-HOME V2 of the LIDDED targeted PCR column-1 + ODTC thermocycler choreography.
#
# Copied from run_targeted_pcr_odtc_1col_full_v2_singlehome_dry.py (the 9-leg, non-lidded
# single-home V2) and extended with the four lid legs, so it is the single-home
# equivalent of run_targeted_pcr_odtc_LIDDED_1col_full_dry.py.
#
...
# Release gates
# ---------------------------------------------------------------------------
CONFIRM_TOKEN = "RUN_TARGETED_PCR_ODTC_LIDDED_SINGLEHOME_DRY"
...
# ---------------------------------------------------------------------------
# PCR1 cleanup geometry (02_targeted_pcr_round1_cleanup_col1_dry_v2_p50low.py)
# ---------------------------------------------------------------------------
TROUGH_BEADS = "A1"
TROUGH_ETOH1 = "A2"
TROUGH_ETOH2 = "A3"
...
def validate_geometry_lock():
    ...
    if actual != CONFIRMED_MOVEMENT_GEOMETRY:
        raise RuntimeError(
            "Targeted PCR movement geometry lock failed; refusing to build or run the deck: "
...
def validate_motion_model_lock():
    ...
    if actual != CONFIRMED_MOTION_MODEL:
        raise RuntimeError(
            "Targeted PCR CellTreat/Corning motion-model lock failed: {} != {}".format(
    ...
    if compensation != CONFIRMED_MOTION_MODEL[-4:]:
        raise RuntimeError(
            "Targeted PCR motion compensation drifted: {} != {}".format(
    ...
    if (
        ...
        raise RuntimeError("Targeted PCR Corning grip-width lock failed")
...
def validate_plr_version():
    try:
        ...
        raise RuntimeError("PyLabRobot is not installed; refusing to build an Targeted PCR run") from exc
...
def assign_unified_deck(lh: LiquidHandler) -> Dict[str, object]:
    ...
    work_plate = CellTreat_96_wellplate_350ul_Fb(name="rail35_pos0_targeted_pcr_work_96wp")
    labware_carrier[WORK_POS] = work_plate
    source_96wp = CellTreat_96_wellplate_350ul_Fb(name="rail35_pos1_targeted_pcr_mm_source_96wp")
    ...
    lid_park = Cor_96_wellplate_360ul_Fb(name="rail35_pos4_targeted_pcr_lid_park", with_lid=True)
...
async def run_full(backend_name):
    ...
    print("")
    print("SUCCESS: full LIDDED Targeted PCR column-1 dry choreography completed in one STAR")
...
def validate_release(args):
    ...
    if args.confirm != CONFIRM_TOKEN:
        raise RuntimeError(
            "Refusing physical Targeted PCR run. Add: --confirm {}".format(CONFIRM_TOKEN)
        )
    if args.acknowledge != DECK_ACK:
        raise RuntimeError(
            "Refusing physical Targeted PCR run until the exact full deck is confirmed. Add: "
            "--acknowledge {}".format(DECK_ACK)
        )
    if args.labware_ack != LABWARE_ACK:
        raise RuntimeError(
            "Refusing physical Targeted PCR run until the CellTreat/Corning combination is "
...
def build_parser():
    parser = argparse.ArgumentParser(
        description=(
            "Single-home LIDDED Targeted PCR column-1 + ODTC landing choreography "
