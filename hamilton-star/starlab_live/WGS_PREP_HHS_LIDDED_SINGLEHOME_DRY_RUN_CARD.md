# WGS preparation + HHS lidded single-home dry run card
...
1. operator-profile lysis dry motion, source column 1 to work column 1
2. operator-profile reaction dry motion, source column 3 to work column 1
...
```bash
python starlab_live/run_wgs_prep_pipetting_hhs_LIDDED_1col_singlehome_dry.py --mode plan
```
...
```bash
./run_on_pi.sh starlab_live/run_wgs_prep_pipetting_hhs_LIDDED_1col_singlehome_dry.py \
  --mode deck
```
...
```bash
python starlab_live/run_wgs_prep_pipetting_hhs_LIDDED_1col_singlehome_dry.py \
  --mode chatterbox
```
...
```text
SUCCESS: continuous WGS preparation + HHS lid/delid/return dry sequence completed.
...
./run_on_pi.sh starlab_live/run_wgs_prep_pipetting_hhs_LIDDED_1col_singlehome_dry.py \
  --mode star \
  --confirm RUN_WGS_PREP_HHS_LIDDED_FULL_DRY \
...
is the WGS preparation release evidence for the future WGS preparation + PCR enrichment team product.
