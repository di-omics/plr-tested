...
# PCR enrichment column-1 full choreography WITH the ODTC thermocycler handoffs baked in.
#
...
def main():
    parser = argparse.ArgumentParser(
        description="PCR enrichment col1 full choreography with ODTC thermocycler handoffs, dry (tips returned)."
    )
    parser.add_argument(
        "--confirm",
        default="",
        help="Required to run the choreography on hardware: --confirm RUN_PCR_ENRICHMENT_ODTC_FULL",
    )
    args = parser.parse_args()

    print("")
    print("FULL PCR ENRICHMENT + ODTC CHOREOGRAPHY, COLUMN 1, DRY (tips returned)")
    ...
    if args.confirm != "RUN_PCR_ENRICHMENT_ODTC_FULL":
        print("")
        print("Refusing to run. This moves the arm through nine transfers, including two ODTC")
        print("round trips and the magnet round trip. Add: --confirm RUN_PCR_ENRICHMENT_ODTC_FULL")
    ...
    print("")
    print("SUCCESS: full PCR enrichment + ODTC column-1 choreography completed. Plate back on rail35 pos0.")
