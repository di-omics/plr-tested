...
# - Transfer the operator-profile stage 3 volume from source 96WP rail35 pos1 col 1
...
#   and moved to the thermocycler for DNA_FRAGMENTATION.
#
# Biology context:
# - Destination/work plate column 1 should already contain WGS preparation product + elution
#   buffer at the operator-approved total volume per well.
# - Source/chilled plate column 1 contains DNA fragmentation master mix.
# - This script adds the operator-profile stage 3 liquid to A1:H1.
#
# Deck:
...
# KEY geometry inherited from x-left validation scaffold:
...
WORK_96WP_DSP_HEIGHT = [4.0] * 8
WORK_96WP_DSP_OFFSETS = [Coordinate(-0.65, 3.35, 0.0)] * 8

SOURCE_96WP_ASP_HEIGHT = [4.0] * 8
...
async def run_dna_fragmentation_addition(lh: LiquidHandler, r: Dict[str, object]):
    ...
    print("\n=== DNA_FRAGMENTATION MASTER MIX ADDITION ONLY ===")
    ...
    print("\nSUCCESS: operator-profile stage 3 addition completed.")
    print("Stop here: manually seal, spin/vortex/spin as needed, and run DNA fragmentation thermocycler program.")


async def main():
    parser = argparse.ArgumentParser(description="WGS preparation V2 operator-profile stage 3 addition smoke test.")
