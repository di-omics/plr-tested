...
# PCR-enrichment stage-2 addition, four-column dry motion proof
#
# Purpose:
# - Keep the existing WGS/sequencing validation deck unchanged.
# - Transfer an operator-defined stage-2 solution to an operator-prepared destination.
...
#
# Wet-method composition, sample assignment, transfer volume, and thermal handoff
# are operator-supplied and intentionally absent from this public file.
...
# 03_pcr_enrichment_round2_mastermix_col1.py as recorded by the hardware validation
...
# returns it. Safe with no reagent on the deck. FATAL with reagent: the tip is planted INSIDE
# each destination reaction to mix (mix Z = +0.5 mm), then returns to the SHARED source well
# at col 1 to aspirate for the next column, carrying indexed product from column 1 into the
# master mix and thence into every later column. The samples are low-input DNA, and
# CLAUDE.md is explicit that carryover is fatal to low-input work. A wet 4-column version
# needs FRESH TIPS PER DESTINATION COLUMN (see MultiRackTipCursor in
# whole_genome_seq_v2_p50only_2rack_4col_discardtips_h7_bo6.py:134) plus a second p50 rack.
# ##########################################################################################
DEST_COLS = [1, 2, 3, 4]

VOL_PCR2_MASTER_MIX = required_positive("pcr_enrichment.round_2_transfer_ul")
REACTION_VOLUME_UL = required_positive("pcr_enrichment.round_2_reaction_volume_ul")
DESTINATION_INPUT_VOLUME_UL = required_positive("pcr_enrichment.round_2_input_volume_ul")
...
PCR2_MM_STEP = Step(
    mode="pcr2-mm",
    label="PCR-enrichment operator-defined stage 2",
...
async def assign_deck(lh: LiquidHandler) -> Dict[str, object]:
    print("Assigning PCR enrichment round 2 deck: WGS/sequencing validation compatible column-1 layout...")
    ...
    work_plate = CellTreat_96_wellplate_350ul_Fb(name="rail35_pos0_pcr_enrichment_round2_dest_96wp")
    source_96wp = CellTreat_96_wellplate_350ul_Fb(name="rail35_pos1_pcr_enrichment_round2_mm_source_96wp")
    ...
    print(
        f"  destination wells start with {DESTINATION_INPUT_VOLUME_UL} uL "
        "operator-prepared input (operator profile)"
    )
    print("  source col 1 A-H contains the operator-defined stage-2 solution")
    print(f"  transfer = {VOL_PCR2_MASTER_MIX} uL x8 by p50, per column, {len(DEST_COLS)} columns "
          f"({VOL_PCR2_MASTER_MIX * len(DEST_COLS)} uL drawn per source well)")
...
async def transfer_pcr2_master_mix(lh: LiquidHandler, r: Dict[str, object], discard_tips: bool, tip_col: int):
    ...
    # returned. Safe DRY ONLY. See the DRY ONLY banner at the top of this file.
    ...
    try:
        ...
        # SURFACE = 1.5 - 1.0 = 0.5 mm above the well bottom. That parameter is a DOWNWARD
        # depth and the sign is proven on hardware, not assumed.
...
async def main():
    parser = argparse.ArgumentParser(
        description="PCR enrichment round 2 common master-mix addition, column 1, WGS/sequencing validation compatible deck."
    )
