...
# PCR enrichment library preparation - operator-volume round 1 reagent addition
#
# Purpose:
# - Keep the existing WGS/validation deck unchanged.
# - Transfer an operator-prepared round 1 reagent using validated motion geometry.
...
async def assign_deck(lh: LiquidHandler) -> Dict[str, object]:
    print("Assigning PCR enrichment operator-volume round 1 deck: WGS/validation compatible layout...")
...
async def main():
    parser = argparse.ArgumentParser(
        description="PCR enrichment round 1 operator-volume transfer using working liquid-handling geometry."
    )
    parser.add_argument(
        "--mode",
