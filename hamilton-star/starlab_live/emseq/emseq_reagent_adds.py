...
# -------------------------------------------------------------
# Source and destination now follow the current hardware-working AmpSeq playbook: both
...
# single-home AmpSeq runner. AmpSeq raised that p50 destination from 0.5 to 1.5 mm after
# 0.5 mm crushed tips into the CellTreat well; do not lower it. The p10 path retains the
# hardware-confirmed PTA/WGA 0.0 mm source / 0.5 mm destination geometry.
#
# The separate iSWAP subprocesses intentionally retain their Cor resource stand-in: that
# is the command model used by the hardware-proven AmpSeq subprocess choreography for the
...
#
# Several EM-seq adds go into much fuller wells than AmpSeq (pcr-mm is 45 uL into 45 uL;
# ligation-mm is 31 uL into 51.5 uL). Matching AmpSeq prevents the known low-Z crush, but
...
# Reused verbatim from the current guarded single-home AmpSeq mastermix path.
