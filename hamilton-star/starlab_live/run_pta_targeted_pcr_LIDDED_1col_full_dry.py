#!/usr/bin/env python3
"""Guarded full PTA/HHS then Targeted PCR/ODTC one-column dry runner.
...
the two current single-home runners in their proven order:
  ...
  1. PTA dry lysis/reaction, HHS plate move, lid on/off, plate return.
  2. Targeted PCR dry PCR1/cleanup/PCR2, ODTC lid cycles, magnet round trip.

Each child owns one LiquidHandler session, homes once, and preserves its own
geometry/model/version locks. The handoff state is explicit: after whole-genome amplification, the work
plate is back at rail35 pos0, the lid is back at rail35 pos4, and HHS is empty;
that is the exact start state required by Targeted PCR. The first composed physical
release is deliberately NOT continuous across that boundary: the operator must
inspect the physical deck and type the exact interphase token before Targeted PCR can
...
PTA_SCRIPT = ROOT / "run_pta_pipetting_hhs_LIDDED_1col_singlehome_dry.py"
TARGETED_PCR_SCRIPT = ROOT / "run_targeted_pcr_odtc_LIDDED_1col_full_v2_singlehome_dry.py"

CONFIRM_TOKEN = "RUN_PTA_TARGETED_PCR_LIDDED_1COL_FULL_DRY"
DECK_ACK = (
...
INTERPHASE_ACK = (
    "PTA_FINAL_PLATE_R35P0_LID_R35P4_HHS_EMPTY_"
    "TIPS_CLEAR_ISWAP_PARKED"
)

PTA_CONFIRM_TOKEN = "RUN_PTA_HHS_LIDDED_FULL_DRY"
PTA_DECK_ACK = "FULL_DRY_DECK_LID_FLAT_HHS_EMPTY"
PTA_LABWARE_ACK = "CELLTREAT_229195_WITH_CORNING_3603_LID"

TARGETED_PCR_CONFIRM_TOKEN = "RUN_TARGETED_PCR_ODTC_LIDDED_SINGLEHOME_DRY"
TARGETED_PCR_DECK_ACK = (
    "R48_P10_P50_P300_R35_WORK_SOURCE_MAGNET_TROUGH_LID_"
    "R20_ODTC_EMPTY_OPEN"
)
TARGETED_PCR_LABWARE_ACK = "CELLTREAT_229195_WORK_SOURCE_CORNING_3603_LID"
...
def preflight_files() -> None:
    missing = [path for path in (PTA_SCRIPT, TARGETED_PCR_SCRIPT) if not path.is_file()]
    if missing:
        raise RuntimeError(
...
def child_command(script: Path, mode: str) -> list[str]:
    ...
    if script == TARGETED_PCR_SCRIPT:
        return base + [
            "--confirm",
            TARGETED_PCR_CONFIRM_TOKEN,
            "--acknowledge",
            TARGETED_PCR_DECK_ACK,
            "--labware-ack",
            TARGETED_PCR_LABWARE_ACK,
        ]
...
def require_interphase_reconciliation(input_fn=None) -> None:
    """Require fresh human evidence after whole-genome amplification, never a pre-supplied CLI flag."""
    if input_fn is None:
        input_fn = input
    banner("MANDATORY PHYSICAL HANDOFF: reconcile PTA final state before Targeted PCR")
    ...
    print("  every Targeted PCR iSWAP path is clear around the installed HHS")
    print("")
    ...
    try:
        ...
        raise RuntimeError(
            "Interphase physical reconciliation could not be read; Targeted PCR was not started."
        ) from exc
    if observed != INTERPHASE_ACK:
        raise RuntimeError(
            "Interphase physical reconciliation did not match; Targeted PCR was not started."
        )


def run_children(mode: str, input_fn=None) -> None:
    run_command(
        "PHASE 1/2: PTA + HHS lidded one-column dry sequence",
        child_command(PTA_SCRIPT, mode),
    )
    if mode == "star":
        require_interphase_reconciliation(input_fn=input_fn)
    run_command(
        "PHASE 2/2: Targeted PCR + ODTC/magnet lidded one-column dry sequence",
        child_command(TARGETED_PCR_SCRIPT, mode),
    )
...
def run_star_preflight() -> None:
    ...
    for script in (PTA_SCRIPT, TARGETED_PCR_SCRIPT):
...
def print_deck() -> None:
    ...
    print("                PTA dry reads columns 1 and 3; Targeted PCR dry reads column 1")
    ...
    print("        physical union and re-checks the PTA-to-Targeted PCR handoff in place")
...
def print_plan(sample_count: int) -> None:
    ...
    print("  phase 1: six-operation PTA/HHS single-home dry sequence")
    print("  phase 2: thirteen-leg Targeted PCR/ODTC/magnet single-home dry sequence")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Guarded PTA/HHS then Targeted PCR/ODTC one-column dry release candidate."
        )
    )
    parser.add_argument(
        "--mode",
...
def main(argv: Iterable[str] | None = None, input_fn=None) -> None:
    ...
    print("")
    print("=" * 92)
    print("SUCCESS: full PTA/HHS -> Targeted PCR/ODTC one-column dry sequence completed.")
