...
TARGETED_PCR_FULL = ROOT / "09_targeted_pcr_full_built_end_to_end_dry.py"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Continuous camera runner: full PTA dry -> HHS return -> full targeted PCR dry."
    )
    parser.add_argument("--confirm", required=True)
    args = parser.parse_args()

    if args.confirm != "RUN_FULL_PTA_HHS_RETURN_TARGETED_PCR_CAMERA":
        raise SystemExit(
            "Refusing to run. Use: --confirm RUN_FULL_PTA_HHS_RETURN_TARGETED_PCR_CAMERA"
        )

    for p in [PTA_FULL, HHS_RETURN, TARGETED_PCR_FULL]:
        if not p.exists():
    ...
    run_phase(
        "3/3 FULL TARGETED PCR DRY CHOREOGRAPHY",
        [
            PYTHON,
            str(TARGETED_PCR_FULL),
        ],
    )
