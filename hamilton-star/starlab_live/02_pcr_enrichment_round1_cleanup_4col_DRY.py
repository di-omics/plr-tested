...
# PCR-enrichment cleanup dry motion scaffold V2 p50-low, four columns
#
# Purpose:
# - Keep the existing WGS/sequencing validation rail35/rail48 carrier approach.
# - Dry/air-move test cleanup motions before any operator-defined wet method.
# - All wet-method volumes and ratios come from the required local operator profile.
# - This script does not mix; wet handoffs remain manual and operator controlled.
...
#
# Reservoir map:
#   A1  = operator-defined cleanup reagent / water mimic
#   A2  = operator-defined wash 1 / water mimic
#   A3  = operator-defined wash 2 / water mimic
#   A4  = operator-defined recovery solution / water mimic
#   A12 = waste
#
# Modes:
#   deck
...
# 02_pcr_enrichment_round1_cleanup_col1_dry_v2_p50low.py as recorded by the
# 13/13 hardware validation; run_all_dry loops its
...
#      reagent; with reagent it is PCR enrichment product carryover across all four samples. A wet 4-col
...
# Required cleanup values from PLR_METHOD_PARAMETERS_FILE.
...
# Reuse the existing tuned sequencing validation cleanup geometry for rail35 pos2 mag + rail35 pos3 trough.
...
# Use this for the operator-defined low-volume cleanup additions.
...
ACTIONS = {
    "beads-add-dry": CleanupAction("beads-add-dry", "Add operator-defined cleanup reagent / water mimic", "add", TROUGH_BEADS, VOL_BEADS),
...
async def assign_deck(lh: LiquidHandler) -> Dict[str, object]:
    ...
    print("\nReservoir map:")
    print(f"  {TROUGH_BEADS} = operator-defined cleanup reagent / water mimic")
    print(f"  {TROUGH_ETOH1} = operator-defined wash 1 / water mimic")
    print(f"  {TROUGH_ETOH2} = optional second wash / water mimic")
    print(f"  {TROUGH_ELUTION} = operator-defined recovery solution / water mimic")
    print(f"  {TROUGH_WASTE} = waste")

    print("\nPCR enrichment round 1 cleanup volumes:")
    print(f"  cleanup reagent add = {VOL_BEADS} uL x8 (operator profile)")
    ...
    print(f"  wash count = {WASH_COUNT} (operator profile)")
...
async def run_ethanol_cycle(lh: LiquidHandler, r: Dict[str, object], discard_tips: bool):
    print(f"\n=== ETHANOL CYCLE DRY: {WASH_COUNT} OPERATOR-PROFILE WASH CYCLE(S) ===")
