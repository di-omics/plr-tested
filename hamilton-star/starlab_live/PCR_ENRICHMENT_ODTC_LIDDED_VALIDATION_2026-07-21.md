# PCR enrichment + ODTC lidded single-home dry validation - 2026-07-21
...
- Runner: `run_pcr_enrichment_odtc_LIDDED_1col_full_v2_singlehome_dry.py`
...
- Dedicated PCR enrichment regression suite: 12 tests passed.
- Combined guarded WGS preparation/PCR enrichment suites: 46 tests passed.
...
- Pi-side normalized iSWAP trace: 20 of 20 `C0PP`/`C0PR` commands matched
  the hardware-proven standalone PCR enrichment component stream exactly.
- No competing STAR Python driver was present immediately before release.

## Physical command

```bash
./run_on_pi.sh starlab_live/run_pcr_enrichment_odtc_LIDDED_1col_full_v2_singlehome_dry.py \
  --mode star \
  --confirm RUN_PCR_ENRICHMENT_ODTC_LIDDED_SINGLEHOME_DRY \
