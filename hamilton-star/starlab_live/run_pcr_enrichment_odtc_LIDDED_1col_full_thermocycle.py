...
# PCR-enrichment column-1 full choreography, LIDDED, with two operator-supplied
# ODTC method profiles woven into the validated hardware flow.
#
# Based on the 2026-07-16 hardware-validated
# run_pcr_enrichment_odtc_LIDDED_1col_full_dry.py choreography (13/13 clean twice).
# The parent marked the two
...
#               legs between LID ON and LID OFF on each ODTC
...
#   homes the door. A supervised hardware check reached the first thermal handoff with
#   the plate and lid seated and confirmed that the mechanical setup remained stable.
#   The operator stopped before running a biological method.
#
# STILL OPEN: this choreography never CLOSES the ODTC door around the thermal leg. The
#   standalone hardware exercise completed regardless of door state, but an open door is
#   not a thermally sound way to run a real method. Worth settling before
#   --thermocycle is pointed at real chemistry.
#
# THE THERMAL LEGS
#   Temperature, duration, cycle-count, lid, and reaction-volume settings are intentionally
#   absent from this public file. ``--round1-operator-profile`` and
#   ``--round2-operator-profile`` must name locally approved ODTC JSON profiles. Each ODTC
#   leg blocks until its operator-supplied method completes.
#
...
#   STEP 2t  ODTC operator method 1, lid sealed               (skipped unless --thermocycle)
...
#   STEP 5   PCR1 cleanup all-dry on the magnet (operator-profile sequence)
...
#   STEP 8t  ODTC operator method 2, lid sealed               (skipped unless --thermocycle)
...
# and the operator to load actual master mix. See pcr_enrichment_state_handoff.md.
#
...
PCR1_MM = ROOT / "01_pcr_enrichment_round1_mastermix_col1.py"
PCR2_MM = ROOT / "03_pcr_enrichment_round2_mastermix_col1.py"
CLEANUP = ROOT / "02_pcr_enrichment_round1_cleanup_col1_dry_v2_p50low.py"
...
def build_steps(mode, odtc_lib, odtc_ip, round1_profile=None, round2_profile=None):
    ...
    parent run_pcr_enrichment_odtc_LIDDED_1col_full_dry.py.
    """
    def thermal(step, stage_label, operator_profile):
        label = (
            f"{step}: ODTC {stage_label} OPERATOR METHOD, lid sealed "
            "(blocks until the cycler finishes)"
        )
        ...
        if mode == "cycle":
            return (label, odtc_thermal_argv(odtc_lib, operator_profile, odtc_ip))
        if mode == "call":
            return (f"{step}: ODTC {stage_label} leg -- DRY, calling the cycler read-only "
                    "(GetStatus). No heat. Real run uses the local operator profile here.",
    ...
    return [
        ...
        thermal("STEP 2t", "stage 1", round1_profile),
        ...
        thermal("STEP 8t", "stage 2", round2_profile),
...
def preflight_odtc(odtc_lib, odtc_ip, operator_profiles=()):
    ...
    for profile in operator_profiles:
        if profile is None or not Path(profile).is_file():
            raise SystemExit(
                "--thermocycle requires two existing operator-owned ODTC JSON profiles; "
                f"not found: {profile!r}"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="LIDDED PCR enrichment col1 full choreography with ODTC handoffs and the two "
                    "ODTC thermal programs baked in (gated behind --thermocycle)."
    )
    parser.add_argument(
        "--confirm",
        default="",
        help="Required to run on hardware: --confirm RUN_PCR_ENRICHMENT_ODTC_LIDDED_FULL",
    )
    parser.add_argument(
        "--thermocycle",
        action="store_true",
    ...
    parser.add_argument(
        "--round1-operator-profile",
        help="operator-owned ODTC JSON profile for the first thermal handoff",
    )
    parser.add_argument(
        "--round2-operator-profile",
        help="operator-owned ODTC JSON profile for the second thermal handoff",
    )
    parser.add_argument(
        "--no-odtc",
        action="store_true",
    ...
    if args.thermocycle and (
        not args.round1_operator_profile or not args.round2_operator_profile
    ):
        parser.error(
            "--thermocycle requires --round1-operator-profile and "
            "--round2-operator-profile"
        )
    ...
    steps = build_steps(
        ...
        odtc_lib,
        args.odtc_ip,
        args.round1_operator_profile,
        args.round2_operator_profile,
    )
    ...
    print("")
    print("FULL PCR ENRICHMENT + ODTC CHOREOGRAPHY, COLUMN 1, LIDDED" + banner)
    ...
    print("")
    if mode == "cycle":
        print("THERMOCYCLE MODE: both locally approved operator profiles run on the real")
        print("cycler, each with the lid sealed on the plate. Tips are still returned and")
        print("no reagent is consumed: this makes the CYCLER real, not the chemistry.")
    ...
    if args.confirm != "RUN_PCR_ENRICHMENT_ODTC_LIDDED_FULL":
        print("")
        ...
        print("Add: --confirm RUN_PCR_ENRICHMENT_ODTC_LIDDED_FULL")
    ...
    if mode in ("cycle", "call"):
        preflight_odtc(
            odtc_lib,
            args.odtc_ip,
            (args.round1_operator_profile, args.round2_operator_profile)
            if mode == "cycle"
            else (),
        )
    ...
    print("")
    if mode == "cycle":
        print("SUCCESS: full LIDDED PCR enrichment + ODTC column-1 choreography completed WITH both")
        ...
        print("SUCCESS: full LIDDED PCR enrichment + ODTC column-1 choreography completed, DRY,")
