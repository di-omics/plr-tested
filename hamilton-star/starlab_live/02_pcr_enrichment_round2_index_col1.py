...
# PCR enrichment library preparation - round 2 indexed-reagent addition, column 1 only
#
# Forked from 01_pcr_enrichment_round1_mastermix_col1.py (verified on hardware 2026-06-15).
# This round 2 front-end mirrors the round 1 swap-source,
# column-1, p50 transfer geometry exactly; only the reagent identity, volume,
# and off-deck handoff change.
#
# Reaction composition, cleanup ratios, sample preparation, and thermal programs
# are intentionally kept in the operator-approved local SOP.
#
...
#
# Reagent layout and row assignments come from the operator-approved local SOP.
# The validated one-to-one channel mapping is retained as liquid-handling geometry.
#
# Transfer volume and the off-deck program identifier come from the required
# operator-approved local method profile.
...
VOL_PCR2_INDEX_MASTER_MIX = required_positive("pcr_enrichment.round_2_transfer_ul")
THERMAL_PROGRAM_ID = required_text("pcr_enrichment.thermal_program_id")

# Reuse the validated validation / PCR1 column-1 P50 geometry verbatim.
...
PCR2_INDEX_STEP = Step(
    mode="pcr2-index",
    label="PCR enrichment round 2 indexed reagent",
    volume_ul=VOL_PCR2_INDEX_MASTER_MIX,
    tip_type="p50",
    manual_prep=(
        "Prepare destination and source wells according to the operator-approved local SOP. "
        "Row-specific source reagents retain the validated one-to-one channel mapping."
    ),
...
async def assign_deck(lh: LiquidHandler) -> Dict[str, object]:
    print("Assigning PCR enrichment round 2 deck: round 1-compatible column-1 swap-source layout...")
    ...
    source_96wp = CellTreat_96_wellplate_350ul_Fb(name="rail35_pos1_pcr_enrichment_round2_index_source_96wp")
    ...
    print("  destination col 1 A-H is prepared according to the approved local SOP")
    print("  source col 1 A-H contains operator-approved row-specific round 2 reagent")
    print(f"  transfer = {VOL_PCR2_INDEX_MASTER_MIX} uL x8 by p50")
...
async def run_dilute_stub(*_args, **_kwargs):
    # Sample preparation has no approved on-deck geometry in this script.
    raise NotImplementedError(
        "dilute mode is intentionally unavailable. Prepare samples according to the "
        "operator-approved local SOP before running pcr2-index."
    )
...
async def main():
    parser = argparse.ArgumentParser(
        description="PCR enrichment round 2 indexed-reagent addition on the validated swap-source deck."
    )
