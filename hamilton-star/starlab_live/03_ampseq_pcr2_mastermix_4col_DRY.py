...
# Amplicon-seq Library Prep - PCR2 common master-mix addition, column 1 only
#
# Purpose:
# - Keep the existing WGS/Bio Validation 0 deck unchanged.
# - Second low-risk ampseq automation target: add common PCR2 master mix.
...
# 03_ampseq_pcr2_mastermix_col1.py at tag ampseq-lidded-inwellmix-2026-07-16 (13/13 clean on
...
#
# The column-loop idiom is taken from 04_ampseq_96wp_pcr1_pcr2_mastermix_DSPH15_DRY.py:210,
...
# master mix and thence into every later column. The sample is single-cell embryo DNA and
# CLAUDE.md is explicit that carryover is fatal to single-cell work. A wet 4-column version
# needs FRESH TIPS PER DESTINATION COLUMN (see MultiRackTipCursor in
# resolve_dna_v2_p50only_2rack_4col_discardtips_h7_bo6.py:134) plus a second p50 rack.
# ##########################################################################################
DEST_COLS = [1, 2, 3, 4]
...
#
# GEOMETRY. CellTreat 350 uL Fb bore = 6.96 mm = 38.046 mm2 = 0.0263 mm per uL, so the
#   25 uL reaction is only 0.657 mm deep. With the tip at 0.5 mm, a 10 uL stroke drops the
#   surface to 0.394 mm, i.e. the tip is above the liquid for the back end of each stroke
#   and bubbles air through it. That still agitates. Drop MIX_VOLUME_UL to ~6.0 to keep the
#   tip submerged for the whole stroke (6 uL leaves the surface at 0.499 mm).
# Sample is single-embryo preimplantation DNA: blowout stays at 10 uL so nothing precious
