...
# -----------------------------------------------------------------------------
# whole-genome sequencing WGS preparation - p300 ethanol wash add/remove focused test
...
#
# Protocol context:
# - Whole-genome sequencing preparation cleanup wash volume and incubation duration
#   come from the operator profile; removal must avoid disturbing beads.
...
# Removal volume comes from the same controlled operator profile.
...
async def main():
    ...
    try:
        ...
        if args.mode == "deck":
            ...
            print(
                "NOTE: Operator-profile incubation after wash add is "
                f"{WASH_INCUBATION_SECONDS} seconds on magnet before removal."
            )
            await remove_from_mag_plate_to_waste(
                lh,
                resources,
