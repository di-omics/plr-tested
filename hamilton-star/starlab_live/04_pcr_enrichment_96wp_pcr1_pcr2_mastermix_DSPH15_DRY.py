...
# PCR enrichment library preparation - round 1/round 2 reagent addition, full 96WP dry demo
#
# Purpose:
# - Keep the existing WGS/validation deck unchanged.
# - Transfer operator-prepared PCR enrichment reagents using validated motion geometry.
# - Template DNA is assumed to already be in destination/work column 1.
# - Transfer volumes come from the operator-approved local method profile.
...
async def assign_deck(lh: LiquidHandler) -> Dict[str, object]:
    print("Assigning PCR enrichment 96-well round 1/round 2 dry deck: WGS/validation compatible layout...")
...
async def main():
    parser = argparse.ArgumentParser(
        description="PCR enrichment 96WP dry demo: round 1/round 2 reagent transfers across destination cols 1-12."
    )
    parser.add_argument(
        "--mode",
