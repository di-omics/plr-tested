...
# pattern as the verified WGS preparation and PCR enrichment master-mix scripts and the methylation_seq/scrnaseq scripts.
#
...
#   gapfill-mix   2.0    p10  5X gap-fill polymerase master mix                       tip-gapfill
...
#                             (100 mM) + 0.5 template-switching reverse transcriptase
...
#   ss-taq        5.9    p50  5X second-strand polymerase master mix                  tip-ss
#   tn5-mix       4.0    p10  2 TAPS buffer + 2 Tn5 (ME-B only, 0.7 uM)               tip-tag
#   pcr-mix      24.0    p50  20 high-fidelity 2X PCR master mix +                    tip-pcr
...
# PCR enrichment/WGS preparation column-1 adds (via the methylation_seq/scrnaseq scripts). No new coordinate is invented.
...
# p10 adds stay within the hardware-confirmed small-volume envelope: up to 6 uL of liquid plus the 7 uL
# blowout air. The combined 13 uL plunger demand sits above the 10 uL tip nominal but is what the
# qualified bo7 motion uses. The liquid-transfer cap is 6 uL
...
# Reused verbatim from the confirmed PCR enrichment/WGS preparation col-1 adds (via methylation_seq/scrnaseq scripts).
...
STEPS: Dict[str, Step] = {
    "gapfill-mix": Step(
        ...
        "Load source col 1 with 5X gap-fill polymerase master mix.",
    ...
    "ivt-mix": Step(
        ...
        "T7 IVT mix: 2 uL NTP (100 mM) + 2 uL 10X T7 reaction buffer + 2 uL "
    ...
    "rt-mix": Step(
        ...
        "with template-switching reverse transcriptase: 4 uL 5X first-strand buffer + "
        "2 uL dNTP (10 mM) + 2 uL DTT (100 mM) + 0.5 uL reverse transcriptase per reaction.",
    ...
    "ss-taq": Step(
        ...
        "Work col 1 holds 23.5 uL. Load source col 1 with 5X second-strand polymerase master mix for "
    ...
    "pcr-mix": Step(
        ...
        "col 1 with the PCR mix: 20 uL high-fidelity 2X PCR master mix + 2 uL index primers "
