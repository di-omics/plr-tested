...
WGS_PREP_FULL = ROOT / "00_wgs_prep_96wp_demo_all12_DSPH15_DRY_ISWAP_R27P2_HHS.py"
HHS_RETURN = ROOT / "test_iswap_plate_rail27pos2_hhs_to_rail35pos0_return.py"
PCR_ENRICHMENT_FULL = ROOT / "09_pcr_enrichment_full_built_end_to_end_dry.py"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Continuous camera runner: full WGS preparation dry -> HHS return -> full PCR enrichment dry."
    )
    parser.add_argument("--confirm", required=True)
    args = parser.parse_args()

    if args.confirm != "RUN_FULL_WGS_PREP_HHS_RETURN_PCR_ENRICHMENT_CAMERA":
        raise SystemExit(
            "Refusing to run. Use: --confirm RUN_FULL_WGS_PREP_HHS_RETURN_PCR_ENRICHMENT_CAMERA"
        )

    for p in [WGS_PREP_FULL, HHS_RETURN, PCR_ENRICHMENT_FULL]:
        if not p.exists():
    ...
    run_phase(
        "1/3 FULL WGS preparation DRY PIPETTING + HHS DROP",
        [
            PYTHON,
            str(WGS_PREP_FULL),
            "--mode",
    ...
    run_phase(
        "3/3 FULL PCR ENRICHMENT DRY CHOREOGRAPHY",
        [
            PYTHON,
            str(PCR_ENRICHMENT_FULL),
        ],
    )
