#!/usr/bin/env python3
"""Guarded full whole-genome amplification/HHS then AmpSeq/ODTC one-column dry runner.
...
the two current single-home runners in their proven order:
  ...
  1. whole-genome amplification dry lysis/reaction, HHS plate move, lid on/off, plate return.
...
AMPSEQ_SCRIPT = ROOT / "run_ampseq_odtc_LIDDED_1col_full_v2_singlehome_dry.py"

CONFIRM_TOKEN = "RUN_PTA_AMPSEQ_LIDDED_1COL_FULL_DRY"
DECK_ACK = (
...
AMPSEQ_CONFIRM_TOKEN = "RUN_AMPSEQ_ODTC_LIDDED_SINGLEHOME_DRY"
AMPSEQ_DECK_ACK = (
    "R48_P10_P50_P300_R35_WORK_SOURCE_MAGNET_TROUGH_LID_"
    "R20_ODTC_EMPTY_OPEN"
)
AMPSEQ_LABWARE_ACK = "CELLTREAT_229195_WORK_SOURCE_CORNING_3603_LID"
...
def preflight_files() -> None:
    missing = [path for path in (PTA_SCRIPT, AMPSEQ_SCRIPT) if not path.is_file()]
    if missing:
        raise RuntimeError(
...
def child_command(script: Path, mode: str) -> list[str]:
    ...
    if script == AMPSEQ_SCRIPT:
        return base + [
            "--confirm",
            AMPSEQ_CONFIRM_TOKEN,
            "--acknowledge",
            AMPSEQ_DECK_ACK,
            "--labware-ack",
            AMPSEQ_LABWARE_ACK,
...
def require_interphase_reconciliation(input_fn=None) -> None:
    ...
    if input_fn is None:
        input_fn = input
    banner("MANDATORY PHYSICAL HANDOFF: reconcile PTA final state before AmpSeq")
    ...
    print("  every AmpSeq iSWAP path is clear around the installed HHS")
    print("")
    ...
    try:
        ...
        raise RuntimeError(
            "Interphase physical reconciliation could not be read; AmpSeq was not started."
        ) from exc
    if observed != INTERPHASE_ACK:
        raise RuntimeError(
            "Interphase physical reconciliation did not match; AmpSeq was not started."
        )
...
def run_children(mode: str, input_fn=None) -> None:
    ...
    run_command(
        "PHASE 2/2: AmpSeq + ODTC/magnet lidded one-column dry sequence",
        child_command(AMPSEQ_SCRIPT, mode),
    )
...
def run_star_preflight() -> None:
    ...
    for script in (PTA_SCRIPT, AMPSEQ_SCRIPT):
...
def print_deck() -> None:
    print("COMBINED PTA + AMPSEQ DRY START DECK")
    ...
    print("                PTA dry reads columns 1 and 3; AmpSeq dry reads column 1")
    ...
    print("        physical union and re-checks the PTA-to-AmpSeq handoff in place")


def print_plan(sample_count: int) -> None:
    validate_sample_count(sample_count)
    blanks = MAX_SAMPLE_COUNT - sample_count
    print("FULL PTA/HHS -> AMPSEQ/ODTC ONE-COLUMN DRY PLAN")
    ...
    print("  phase 2: thirteen-leg AmpSeq/ODTC/magnet single-home dry sequence")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Guarded PTA/HHS then AmpSeq/ODTC one-column dry release candidate."
        )
    )
    parser.add_argument(
        "--mode",
...
def main(argv: Iterable[str] | None = None, input_fn=None) -> None:
    ...
    print("")
    print("=" * 92)
    print("SUCCESS: full PTA/HHS -> AmpSeq/ODTC one-column dry sequence completed.")
