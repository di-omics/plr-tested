#!/usr/bin/env python3
"""Continuous single-home whole-genome amplification plus lidded HHS dry engineering runner.

This runner combines the individually hardware-proven dry stages from
``run_pta_pipetting_hhs_LIDDED_1col_dry.py`` into one LiquidHandler session:

  1. Lysis dry motion, source column 1 -> work column 1, 3.0 uL x8
  2. Reaction dry motion, source column 3 -> work column 1, 6.0 uL x8
...
Physical component evidence recorded 2026-07-21:
  ...
  - whole-genome amplification dry lysis/reaction plus corrected HHS forward: pass
...
STAGED_RUNNER = ROOT / "run_pta_pipetting_hhs_LIDDED_1col_dry.py"

CONFIRM_TOKEN = "RUN_PTA_HHS_LIDDED_FULL_DRY"
...
def load_staged_runner() -> ModuleType:
    ...
    module_name = "pta_hhs_lidded_staged_for_singlehome"
...
def assign_unified_deck(lh, pta):
    ...
    tip_carrier = TIP_CAR_480_A00(name="pta_full_tip_carrier_r48")
    plate_carrier = PLT_CAR_L5AC_A00(name="pta_full_plate_carrier_r35")
    hhs_carrier = PLT_CAR_L5AC_A00(name="pta_full_hhs_carrier_r27")
    ...
    p10_tips = pta.make_p10_tips("pta_full_r48p0_p10_filter_tips")
    p50_tips = pta.make_p50_tips("pta_full_r48p1_p50_filter_tips")
    tip_carrier[staged.P10_TIP_POS] = p10_tips
    tip_carrier[staged.P50_TIP_POS] = p50_tips

    work_plate = CellTreat_96_wellplate_350ul_Fb(name="pta_full_work_plate_r35p0")
    source_plate = CellTreat_96_wellplate_350ul_Fb(name="pta_full_source_plate_r35p1")
    park_plate = Cor_96_wellplate_360ul_Fb(
        name="pta_full_lid_park_r35p4",
        with_lid=True,
    )
...
async def run_choreography(lh, resources, pta) -> None:
    ...
    staged.assert_stage_state(resources, "pta-forward", "before")
    assert_sites_pristine(resources, pristine_sites, "initial deck")

    banner("1/6 PTA DRY LYSIS: source column 1 -> work column 1")
    await pta.transfer_step(
        lh,
        resources,
        staged.dry_transfer_step(pta, "lysis", source_col=1),
        False,
        tip_col=1,
        source_col=1,
        dest_col=1,
    ...
    banner("2/6 PTA DRY REACTION: source column 3 -> work column 1")
    await pta.transfer_step(
        lh,
        resources,
        staged.dry_transfer_step(pta, "reaction", source_col=3),
        False,
        tip_col=2,
        source_col=3,
        dest_col=1,
    ...
    staged.assert_stage_state(resources, "pta-forward", "after")
...
async def run_full(backend_name: str) -> None:
    ...
    try:
        ...
        setup_complete = True
        resources = assign_unified_deck(lh, pta)
        print(
        ...
        await run_choreography(lh, resources, pta)
        choreography_succeeded = True
        print("")
        print("SUCCESS: continuous PTA + HHS lid/delid/return dry sequence completed.")
...
def run_deck() -> None:
    ...
    resources = assign_unified_deck(shell, pta)
    staged.assert_stage_state(resources, "pta-forward", "before")
...
def print_plan() -> None:
    staged.validate_geometry_lock()
    print("CONTINUOUS SINGLE-HOME PTA + HHS LIDDED DRY PLAN")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Continuous single-home one-column PTA plus lidded HHS dry runner."
    )
    parser.add_argument(
        "--mode",
