# scRNA-seq motion scaffold

This directory contains a simulation-first, single-column Hamilton/ODTC scaffold
for scRNA-seq. It intentionally describes only generic numbered stages and
magnetic-cleanup handoffs.
...
- `scrnaseq_reagent_adds.py --mode stage-1` through `stage-9`
- `scrnaseq_cleanup.py --cleanup cleanup-1|cleanup-2|cleanup-3`
- `run_scrnaseq_odtc_1col_full_dry.py --print`
- ODTC handoffs `scrnaseq-stage-1` through `scrnaseq-stage-7`

The rail assignments, plate models, tip-capacity limits, aspiration/dispense
heights, offsets, blowout values, settle times, and iSWAP pickup/drop offsets
are hardware motion data and remain in code. These scRNA-seq liquid transfers
...
./hamilton-star/run_on_pi.sh \
  starlab_live/scrnaseq/scrnaseq_reagent_adds.py \
