...
# whole-genome sequencing WGS preparation Automation - V2 rail35 integrated scaffold
# Integrates:
...
P10_STEPS: List[Tuple[int, float, str]] = [
    (SRC_LYSIS_COL, required_positive("wgs.stage_1_volume_ul"), "operator stage 1"),
    (SRC_REACTION_COL, required_positive("wgs.stage_2_volume_ul"), "operator stage 2"),
    (SRC_DNA_FRAGMENTATION_COL, required_positive("wgs.stage_3_volume_ul"), "operator stage 3"),
    (SRC_END_REPAIR_COL, required_positive("wgs.stage_4_volume_ul"), "operator stage 4"),
    (SRC_ADAPTER_COL, required_positive("wgs.stage_5_volume_ul"), "operator stage 5"),
    (SRC_LIGATION_MIX_COL, required_positive("wgs.stage_6_volume_ul"), "operator stage 6"),
]
P50_STEPS: List[Tuple[int, float, str]] = [
    (SRC_LIBRARY_PCR_COL, required_positive("wgs.stage_7_volume_ul"), "operator stage 7"),
]
...
async def run_wash_sequence(lh, r, dest_cols, discard_tips):
    print(
        f"\n=== V2 WASH SEQUENCE ON MAGNET: {WASH_COUNT} operator-profile cycle(s) "
        f"with {ETHANOL_INCUBATION_SECONDS}-second incubations ==="
    )
    wash_steps = ((TROUGH_ETOH1, 1, 2), (TROUGH_ETOH2, 3, 4))
    for source_well, add_tip_col, remove_tip_col in wash_steps[:WASH_COUNT]:
        await p300_add_from_reservoir_to_mag(
            lh, r, source_well, dest_cols, VOL_ETOH,
            tip_col=add_tip_col, discard_tips=discard_tips,
        )
        print(f"Incubating on magnet for {ETHANOL_INCUBATION_SECONDS} seconds...")
        await asyncio.sleep(ETHANOL_INCUBATION_SECONDS)
        await p300_remove_from_mag_to_waste(
            lh, r, dest_cols, TROUGH_WASTE, VOL_ETOH_REMOVE,
            tip_col=remove_tip_col, discard_tips=discard_tips,
        )
