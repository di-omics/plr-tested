...
#   heights (asp/dsp 1.5) and the PCR enrichment-derived XY (-0.68, 3.22) held across plate
...
#
# Deck (rail35 / rail48, Validation layout):
...
# IN-WELL MIX (firmware Mix), lifted from the PCR enrichment geometry documented
...
# 2026-07-16; it is now the same firmware Mix the PCR enrichment build proved.
#
...
# same proven-safe geometry as PCR enrichment. param 2.0 would give -0.5 mm = tips INTO the
...
MIX_VOLUME_UL = 80.0         # 40% of the 200 uL working volume. CellTreat 350 uL Fb bore =
                             ...
                             # at 0.5 mm stays submerged for the whole stroke (unlike PCR enrichment's
                             # shallow 25 uL reaction, which had to drop to 6 uL to stay under).
MIX_FLOW_RATE = 50.0         # uL/s. PCR enrichment uses 10 because its 10 uL stroke at 50 finished in
...
# Work-plate XY, seeded from the validated PCR enrichment work dispense
...
P300_PLATE_DSP_HEIGHT = 1.5   # PROVEN, DO NOT LOWER. PCR enrichment raised this 0.5 -> 1.5 on
...
P300_MIX_BLOWOUT_AIR_VOLUME = 10.0   # heavy blowout after the in-well mix so nothing is left
                                     # in the tip, per the PCR enrichment build. 200 uL is deep enough
                                     # that the shallow-well splash risk PCR enrichment worried about
...
async def run_serial(lh: LiquidHandler, r: Dict[str, object], discard_tips: bool, first_rack_col: int, mix_cycles: int):
    ...
    for src in DILUTION_COLS[:-1]:   # 1..10
        ...
        try:
            ...
            await finish_tips(lh, discard_tips)
        # Only burn a fresh rack column when we are actually discarding. This is the PCR enrichment
