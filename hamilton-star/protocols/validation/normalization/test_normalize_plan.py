...
# Deliberately generic linear fixture, unrelated to any assay insert or run card.
stds = [0.0, 1.0, 2.0]
rfu = [4.0 + 3.0 * c for c in stds]
fit, wells = concentrations_from_rfu(
    standard_conc_ng_per_ml=stds,
    standard_rfu=rfu,
    sample_rfu_by_well={"A1": 8.5},
    assay_dilution=2.0,
)
a1 = wells["A1"]
check("curve fit slope recovered", approx(fit.slope, 3.0, 1e-6))
check("assay dilution multiplied back", approx(a1.conc_ng_per_ul, 0.003, 1e-6), f"got {a1.conc_ng_per_ul}")
check("ng/mL -> ng/uL is /1000", approx(ng_per_ml_to_ng_per_ul(3.0), 0.003))
