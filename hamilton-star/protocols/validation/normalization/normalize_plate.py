...
Consumes a per-well concentration source (a real Tecan fluorescent dsDNA assay read is the goal,
...
Concentration source (pick one), all in ng/uL after any assay-dilution multiply-back:
  ...
  --rfu-csv FILE    rows "well,rfu"; assay values load from the required local profile
...
from operator_parameters import required_number_list, required_positive  # noqa: E402
...
# work-plate XY: validated PCR enrichment work dispense (Y must stay > 3.20, blacklisted)
...
def assay_profile():
    """Load the complete assay calibration from the operator-owned local profile."""

    concentrations = required_number_list(
        "normalization.standard_concentrations_ng_per_ml",
        minimum_items=2,
        nonnegative=True,
    )
    rfus = required_number_list(
        "normalization.standard_rfu",
        minimum_items=2,
        nonnegative=True,
    )
    if len(concentrations) != len(rfus):
        raise SystemExit(
            "normalization standard concentration and RFU arrays must have equal length"
        )
    return {
        "standard_concentrations_ng_per_ml": concentrations,
        "standard_rfu": rfus,
        "assay_dilution": required_positive("normalization.assay_dilution"),
        "excitation_nm": required_positive("normalization.excitation_nm"),
        "emission_nm": required_positive("normalization.emission_nm"),
    }
...
def get_concentrations(args) -> Dict[str, float]:
    ...
    if args.rfu_csv:
        ...
        print(
            "Assay calibration: operator-owned local profile "
            f"({profile['excitation_nm']:g}/{profile['emission_nm']:g} nm)"
        )
        _, wells = NP.concentrations_from_rfu(
            profile["standard_concentrations_ng_per_ml"],
            profile["standard_rfu"],
            read_rfu_csv(args.rfu_csv),
            assay_dilution=profile["assay_dilution"],
        )
...
async def main():
    ...
    ap.add_argument(
        "--rfu-csv",
        help="rows well,rfu; curve, dilution, and wavelengths come from PLR_METHOD_PARAMETERS_FILE",
    )
