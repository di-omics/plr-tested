...
RUNNER_PATH = (
    ...
    / "starlab_live"
    / "run_pta_pipetting_hhs_LIDDED_1col_dry.py"
)


def load_runner(module_name="pta_hhs_lidded_dry_runner_test"):
...
class PlanTests(unittest.TestCase):
    def test_import_and_plan_do_not_import_pylabrobot(self):
        ...
        with patch("builtins.__import__", side_effect=deny_plr):
            isolated = load_runner("pta_hhs_lidded_plan_inert_test")
            output = io.StringIO()
            with redirect_stdout(output):
                isolated.print_plan()
        self.assertIn("PTA + HHS LIDDED", output.getvalue())

    def test_stage_order_and_corrected_geometry_are_locked(self):
        self.assertEqual(
            runner.STAGE_ORDER,
            ("pta-forward", "lid-on", "delid", "plate-return"),
        )
    ...
    def test_dry_transfer_prompts_name_the_actual_empty_source_columns(self):
        pta = runner.load_pta_module()
        lysis = runner.dry_transfer_step(pta, "lysis", source_col=1)
        reaction = runner.dry_transfer_step(pta, "reaction", source_col=3)
...
class ReleaseGateTests(unittest.TestCase):
    ...
    def test_wrong_confirm_is_refused(self):
        with self.assertRaisesRegex(RuntimeError, "RUN_PTA_FORWARD_DRY"):
            runner.validate_release(self.args("pta-forward", confirm="wrong"))
    ...
    def test_forward_requires_dry_start_deck_acknowledgement(self):
        ...
        with self.assertRaisesRegex(RuntimeError, token):
            runner.validate_release(self.args("pta-forward"))
        runner.validate_release(self.args("pta-forward", acknowledge=token))
...
class ResourceTreeTests(unittest.TestCase):
    ...
    def stage_deck(stage):
        ...
        return runner.assign_full_stage_deck(shell, stage, pta)
...
class FailureCleanupTests(unittest.TestCase):
    def test_move_failure_skips_auto_park_and_disconnects(self):
        ...
        with (
            ...
            patch.object(runner, "load_pta_module", return_value=object()),
