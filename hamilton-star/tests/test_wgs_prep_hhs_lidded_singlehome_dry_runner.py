...
RUNNER_PATH = (
    ...
    / "starlab_live"
    / "run_wgs_prep_pipetting_hhs_LIDDED_1col_singlehome_dry.py"
)


def load_runner(module_name="wgs_prep_hhs_lidded_singlehome_test"):
...
class PlanAndGateTests(unittest.TestCase):
    def test_import_and_plan_do_not_import_pylabrobot(self):
        ...
        with patch("builtins.__import__", side_effect=deny_plr):
            isolated = load_runner("wgs_prep_hhs_singlehome_inert_test")
...
class UnifiedGeometryTests(unittest.TestCase):
    def setUp(self):
        self.wgs_prep = runner.staged.load_wgs_prep_module()
        self.handler = RecordingHandler()
        self.resources = runner.assign_unified_deck(self.handler, self.wgs_prep)
    ...
    def test_unified_deck_is_pristine_and_truthful(self):
        runner.staged.assert_stage_state(self.resources, "wgs_prep-forward", "before")
    ...
    def test_complete_choreography_has_exact_geometry_and_parent_transitions(self):
        ...
        with patch.object(self.wgs_prep, "transfer_step", new=fake_transfer):
            asyncio.run(runner.run_choreography(self.handler, self.resources, self.wgs_prep))
        ...
        self.assertEqual(
            ...
            [
                (
                    "lysis",
                    self.wgs_prep.STEPS["lysis"].volume_ul,
                    "p10",
                    False,
                    1,
                    1,
                ...
                (
                    "reaction",
                    self.wgs_prep.STEPS["reaction"].volume_ul,
                    "p10",
                    False,
                    2,
                    3,
    ...
    def test_plate_move_failure_restores_local_and_site_coordinates(self):
        ...
        resources = runner.assign_unified_deck(failing, self.wgs_prep)
        ...
        runner.staged.assert_stage_state(resources, "wgs_prep-forward", "before")


class LifecycleTests(unittest.TestCase):
    def test_run_full_uses_one_setup_one_deck_four_moves_one_park_stop(self):
        wgs_prep = runner.staged.load_wgs_prep_module()
        handler = RecordingHandler()
        transfer = AsyncMock()
        real_assign = runner.assign_unified_deck
        with (
            patch.object(runner.staged, "load_wgs_prep_module", return_value=wgs_prep),
            ...
            patch.object(wgs_prep, "transfer_step", transfer),
        ...
        for fail_transfer in (1, 2):
            with self.subTest(fail_transfer=fail_transfer):
                ...
                with (
                    patch.object(runner.staged, "load_wgs_prep_module", return_value=wgs_prep),
                    patch.object(runner.staged, "make_handler", return_value=handler),
                    patch.object(wgs_prep, "transfer_step", new=transfer_fault),
                    redirect_stdout(StringIO()),
                ):
