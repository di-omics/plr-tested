"""Offline tests for the guarded full PTA-to-Targeted PCR dry composition."""
...
RUNNER = (
    ...
    / "starlab_live"
    / "run_pta_targeted_pcr_LIDDED_1col_full_dry.py"
)


def load_runner():
    name = "pta_targeted_pcr_lidded_full_dry_under_test"
...
class CombinedDryRunnerTests(unittest.TestCase):
    ...
    def test_deck_mode_runs_only_both_connection_free_child_previews(self):
        ...
        self.assertEqual(
            ...
            self.runner.child_command(self.runner.PTA_SCRIPT, "deck"),
        )
        self.assertEqual(
            run.call_args_list[1].args[0],
            self.runner.child_command(self.runner.TARGETED_PCR_SCRIPT, "deck"),
        )
        for call in run.call_args_list:
            self.assertTrue(call.kwargs["check"])

    def test_chatterbox_runs_pta_then_targeted_pcr_without_star_tokens(self):
        with mock.patch.object(self.runner.subprocess, "run") as run:
        ...
        self.assertEqual(
            ...
            [
                self.runner.child_command(self.runner.PTA_SCRIPT, "chatterbox"),
                self.runner.child_command(self.runner.TARGETED_PCR_SCRIPT, "chatterbox"),
            ],
        )
    ...
    def test_star_runs_both_deck_preflights_before_exact_guarded_children(self):
        ...
        self.assertEqual(
            ...
            [
                self.runner.child_command(self.runner.PTA_SCRIPT, "deck"),
                self.runner.child_command(self.runner.TARGETED_PCR_SCRIPT, "deck"),
                self.runner.child_command(self.runner.PTA_SCRIPT, "star"),
                self.runner.child_command(self.runner.TARGETED_PCR_SCRIPT, "star"),
            ],
        )
    ...
    def test_child_failure_stops_before_the_next_phase(self):
        ...
        failure = subprocess.CalledProcessError(1, "pta")
    ...
    def test_wrong_interphase_observation_blocks_targeted_pcr_after_pta(self):
        argv = [
            "--mode",
            "star",
            "--confirm",
        ...
        with mock.patch.object(self.runner.subprocess, "run") as run:
            with self.assertRaisesRegex(RuntimeError, "Targeted PCR was not started"):
    ...
    def test_missing_interphase_input_blocks_targeted_pcr_after_pta(self):
    ...
    def test_child_star_argv_carries_each_runner_exact_release_tokens(self):
        pta = self.runner.child_command(self.runner.PTA_SCRIPT, "star")
        targeted_pcr = self.runner.child_command(self.runner.TARGETED_PCR_SCRIPT, "star")
        self.assertEqual(
            pta[-6:],
            [
                "--confirm",
                self.runner.PTA_CONFIRM_TOKEN,
                "--acknowledge",
                self.runner.PTA_DECK_ACK,
                "--labware-ack",
                self.runner.PTA_LABWARE_ACK,
            ],
        )
        self.assertEqual(
            targeted_pcr[-6:],
            [
                "--confirm",
                self.runner.TARGETED_PCR_CONFIRM_TOKEN,
                "--acknowledge",
                self.runner.TARGETED_PCR_DECK_ACK,
                "--labware-ack",
                self.runner.TARGETED_PCR_LABWARE_ACK,
            ],
        )
