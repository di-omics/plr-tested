"""Offline tests for the guarded full PTA-to-AmpSeq dry composition."""
...
RUNNER = (
    ...
    / "starlab_live"
    / "run_pta_ampseq_LIDDED_1col_full_dry.py"
)


def load_runner():
    name = "pta_ampseq_lidded_full_dry_under_test"
...
class CombinedDryRunnerTests(unittest.TestCase):
    ...
    def test_deck_mode_runs_only_both_connection_free_child_previews(self):
        ...
        self.assertEqual(
            ...
            self.runner.child_command(self.runner.AMPSEQ_SCRIPT, "deck"),
        )
        for call in run.call_args_list:
            self.assertTrue(call.kwargs["check"])

    def test_chatterbox_runs_pta_then_ampseq_without_star_tokens(self):
        with mock.patch.object(self.runner.subprocess, "run") as run:
        ...
        self.assertEqual(
            ...
            [
                ...
                self.runner.child_command(self.runner.AMPSEQ_SCRIPT, "chatterbox"),
            ],
        )
    ...
    def test_star_runs_both_deck_preflights_before_exact_guarded_children(self):
        ...
        self.assertEqual(
            ...
            [
                ...
                self.runner.child_command(self.runner.AMPSEQ_SCRIPT, "star"),
            ],
        )
    ...
    def test_wrong_interphase_observation_blocks_ampseq_after_pta(self):
        argv = [
            "--mode",
            "star",
            "--confirm",
        ...
        with mock.patch.object(self.runner.subprocess, "run") as run:
            with self.assertRaisesRegex(RuntimeError, "AmpSeq was not started"):
    ...
    def test_missing_interphase_input_blocks_ampseq_after_pta(self):
    ...
    def test_child_star_argv_carries_each_runner_exact_release_tokens(self):
        ...
        ampseq = self.runner.child_command(self.runner.AMPSEQ_SCRIPT, "star")
        self.assertEqual(
            pta[-6:],
            [
                "--confirm",
        ...
        self.assertEqual(
            ...
            [
                "--confirm",
                self.runner.AMPSEQ_CONFIRM_TOKEN,
                "--acknowledge",
                self.runner.AMPSEQ_DECK_ACK,
                "--labware-ack",
                self.runner.AMPSEQ_LABWARE_ACK,
            ],
        )
