...
class PlanningHandler(BaseHTTPRequestHandler):
    ...
    server_version = f"PTAAmpSeqPlanner/{__version__}"
    ...
    def do_GET(self) -> None:
        ...
        if path in ("/", "/index.html"):
            index = resources.files("pta_ampseq_app").joinpath("static").joinpath("index.html")
...
def main(argv: Optional[list] = None) -> int:
    parser = argparse.ArgumentParser(
        prog="pta-ampseq-app",
        description="Planning-only local PTA + AmpSeq bench wizard.",
    )
    ...
    print(f"PTA + AmpSeq bench planner: http://127.0.0.1:{port}")
