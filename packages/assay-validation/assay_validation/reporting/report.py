...
def _wgs_prep_body(data: dict) -> str:
    return (
        ...
        f"<div class='k'>Stage 1 / stage 2</div><div class='v mono'>{_esc(data.get('stage_1_ul'))} uL / {_esc(data.get('stage_2_ul'))} uL</div>"
        "</div>"
...
def _pcr_enrichment_body(data: dict) -> str:
    ...
    anneal_txt = f"{anneal} C"
    return (
        "<div class='kv'>"
        f"<div class='k'>Locus</div><div class='v'>{_esc(data.get('locus'))} ({_esc(data.get('pcr_product_bp'))} bp)</div>"
...
def _handoff_body(data: dict) -> str:
    ...
    kv = (
        ...
        f"<div class='k'>Analysis</div><div class='v'>{_esc(data.get('analysis_intent'))}</div>"
        "</div>"
    )
...
<title>Assay-validation dossier - {_esc(cfg.run_id)}</title>
<style>{CSS}</style></head>
<body><div class="wrap">
  <div class="eyebrow">Assay-validation package</div>
  <h1>{_esc(cfg.run_id)}</h1>
  <p class="sub">WGS preparation + PCR enrichment, QC-gated, for confirming {_esc(cfg.analysis_type.value.replace('_',' '))} at {_esc(cfg.locus.name)}.</p>
  <div class="meta">
    <span>Operator <b>{_esc(cfg.operator)}</b></span>
    <span>Mode <b>{_esc(cfg.mode.value)}</b></span>
    <span>Locus <b>{_esc(cfg.locus.name)} ({_esc(cfg.locus.pcr_product_bp)} bp)</b></span>
  ...
  <div class="foot">di-omics &#183; plr-tested &#183; assay-validation</div>
</div></body></html>"""
