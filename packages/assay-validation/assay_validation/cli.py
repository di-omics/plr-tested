...
cli.py - the operator entrypoint.
    ...
    assay-validate run  <manifest> [--out DIR] [--hardware] [--poor-deck]
    assay-validate plan <manifest>
    assay-validate demo [--out DIR]
...
Also runnable as `python -m assay_validation ...`.
"""
...
def cmd_run(args) -> int:
    try:
        ...
        return 1
    if args.hardware:
        if cfg.method.profile_kind is not ProfileKind.OPERATOR:
            print(
                "manifest error: --hardware requires method profile_kind='operator'; "
                "synthetic profiles are water-only",
                file=sys.stderr,
            )
            return 1
...
def cmd_plan(args) -> int:
    ...
    print(f"analysis_type  : {cfg.analysis_type.value}")
    print(f"locus      : {cfg.locus.name}  PCR product {cfg.locus.pcr_product_bp} bp")
    print(f"deck       : {cfg.deck.name}")
    print(f"method     : {cfg.method.profile_kind.value} ({cfg.method.parameter_source})")
    print(f"PCR1 anneal: {cfg.method.pcr1_anneal_c} C")
    print(f"PCR2 cycles: {cfg.method.pcr2_cycles}")
    print("samples    :")
    ...
    print(f"  Gate 1 post-WGS preparation yield         >= {ac.wgs_prep_yield_min_ng} ng  "
          f"(uniformity CV <= {ac.wgs_prep_uniformity_cv_max_percent} %)")
    print(f"  Gate 2 post-PCR-enrichment conc       {ac.pcr_enrichment_conc_min_ng_per_ul} to "
          f"{ac.pcr_enrichment_conc_max_ng_per_ul} ng/uL")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="assay-validate",
                                description="QC-gated WGS preparation + PCR enrichment for gene-assay validation")
    p.add_argument("--version", action="version", version=f"assay-validate {__version__}")
