...
package has two:
  ...
  compute tier   the simulation and all the QC math. Needs nothing but Python. If this
                 is green, the site can run `assay-validate demo` right now and read a
  ...
  hardware tier  driving the STAR, ODTC, and Tecan. Needs the PyLabRobot fork, the Pi
                 ...
                 `assay-validate doctor --hardware` lists what is still missing.
...
def _compute_checks() -> List[Check]:
    ...
    core_err = _try_import("assay_validation.orchestrator")
    checks.append(Check(
...
def _simulation_selftest() -> tuple:
    try:
        ...
        cfg = build_run({
            ...
            "locus": {"name": "L", "pcr_product_bp": 200},
            "method": {
...
def format_report(checks: List[Check], hardware: bool) -> str:
    ...
    lines.append(f"assay-validate doctor  ({tier} tier)")
    lines.append("")
    for c in checks:
    ...
    if not hardware:
        ...
        if compute_ok:
            lines.append("compute tier ready: run `assay-validate demo`.")
            lines.append("For a real run, check the hardware tier: `assay-validate doctor --hardware`.")
        else:
        ...
        if n_missing == 0:
            ...
            lines.append(f"{n_missing} item(s) to resolve before a hardware run. "
                         "Simulation works regardless: `assay-validate demo`.")
    return "\n".join(lines)
