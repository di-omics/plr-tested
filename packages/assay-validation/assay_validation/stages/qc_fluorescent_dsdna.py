...
The same fluorescent dsDNA assay quant runs after WGS preparation and PCR enrichment; only the acceptance
criterion differs. So this is one stage parameterized by checkpoint:
  ...
  POST_PCR_ENRICHMENT  per-well library concentration must sit inside the loading window for
               even pooling and fragment analyzer. -> PROCEED_SUBSET on the wells in window.

The assay itself: a reference dsDNA standard curve is read on the same plate, its linearity
...
class FluorescentDsDNAQC(Stage):
    ...
    def _read_standard_curve(self, ctx: StageContext):
        profile = ctx.config.fluorescent_dsdna
        concs: List[float] = list(profile.standards_ng_per_ml)
        ...
        read = ctx.tecan.read(
            ctx.config.run_id,
            f"fluorescent_dsdna_standards_{self.checkpoint.value}",
            truth,
            profile.excitation_nm,
            profile.emission_nm,
        )
    ...
    def run(self, ctx: StageContext) -> StageResult:
        ...
        method = ctx.config.method
        dilution = (
            method.wgs_qc_dilution
            if self.checkpoint is Checkpoint.POST_WGS_PREP
            else method.pcr_qc_dilution
        )
        assay_volume = (
            method.wgs_product_volume_ul
            if self.checkpoint is Checkpoint.POST_WGS_PREP
            else method.pcr_library_volume_ul
        )
        (
            passing_center,
            failing_center,
        ...
        read = ctx.tecan.read(
            ctx.config.run_id,
            f"fluorescent_dsdna_samples_{self.checkpoint.value}",
            truth,
            profile.excitation_nm,
            profile.emission_nm,
        )
