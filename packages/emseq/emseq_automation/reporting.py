...
<h1>EM-seq v2 + UltraShear run dossier</h1>
<p><span class="badge">{_esc(outcome.status.value)}</span> {_esc(outcome.message)}</p>
<table><tr><th>Run</th><th>Operator</th><th>Mode</th><th>Input route</th><th>Shear</th><th>PCR</th></tr>
<tr><td>{_esc(outcome.config.run_id)}</td><td>{_esc(outcome.config.operator)}</td>
<td>{_esc(outcome.config.mode.value)}</td><td>{_esc(outcome.config.input_tier.value)}</td>
<td>{outcome.config.shear_minutes:g} min</td><td>{outcome.config.pcr_cycles} cycles</td></tr></table>
...
def render_run_card(outcome: RunOutcome) -> str:
    lines = [
        f"# EM-seq run card: {outcome.config.run_id}",
        "",
