...
def _print_plan(config) -> None:
    print(f"EM-seq v2 + UltraShear plan: {config.run_id}")
    print(f"route={config.input_tier.value}  shear={config.shear_minutes:g} min  "
          f"PCR={config.pcr_cycles} cycles  samples={len(config.samples)}")
    print("")
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(
        prog="emseq-run",
        description="QC-gated NEBNext EM-seq v2 + UltraShear automation",
    )
