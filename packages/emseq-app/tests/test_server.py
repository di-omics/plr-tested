...
from emseq_app.server import create_server
...
class PlanningServerTests(unittest.TestCase):
    ...
    def test_state_is_planning_only_with_narrow_dry_evidence(self):
        ...
        self.assertEqual(payload["runtime"]["observed_physical_dry_minutes"], 67)
        self.assertEqual(payload["runtime"]["default_thermal_hold_minutes"], 389)
        self.assertFalse(payload["runtime"]["multi_column_estimate_available"])
        deck_locations = {
    ...
    def test_index_explains_protocol_scope_and_keeps_hardware_disabled(self):
        ...
        self.assertIn("about 6 h 29 min", html)
        self.assertIn('id="runtime-note"', html)
