"""
stages/targeted_pcr.py - pull the edit locus out of the PTA product and index it.
...
to the loading window. It runs only on the wells that passed the post-whole-genome amplification yield gate, so
...
programs map to the ODTC's targeted-pcr-round1 / targeted-pcr-round2. Master-mix volumes and SPRI ratios
...
No gate here. Gate 2 (post-targeted PCR PicoGreen) follows and decides what reaches sequencing.
"""
...
PCR1_MM_UL = transcribed(
    22.5, "01_targeted_pcr_round1_mastermix_col1.py: 22.5 uL complete PCR1 master mix",
    unit="uL", name="pcr1_mm_volume",
)
PCR2_MM_UL = transcribed(
    20.5, "03_targeted_pcr_round2_mastermix_col1.py: 20.5 uL common PCR2 master mix",
    unit="uL", name="pcr2_mm_volume",
)


class TargetedPCR(Stage):
    name = "targeted_pcr"
    title = "Targeted PCR library preparation (edit locus)"
    ...
    def run(self, ctx: StageContext) -> StageResult:
        ...
        ctx.star.add_mastermix("PCR1", float(PCR1_MM_UL.value), "p50",
                               "01_targeted_pcr_round1_mastermix_col1.py")
        ctx.star.iswap_move("work_plate", "odtc")
        pcr1_ref = ctx.odtc.run_program("targeted-pcr-round1", anneal_c=anneal)
        ...
        ctx.star.add_mastermix("PCR2", float(PCR2_MM_UL.value), "p50",
                               "03_targeted_pcr_round2_mastermix_col1.py")
        ctx.star.iswap_move("work_plate", "odtc")
        pcr2_ref = ctx.odtc.run_program("targeted-pcr-round2", num_cycles=ctx.config.pcr2_cycles)
        ctx.star.iswap_move("odtc", "work_plate")
        ...
        return StageResult(
            ...
            data={
                ...
                "locus": locus.name,
                "target_product_bp": locus.target_product_bp,
