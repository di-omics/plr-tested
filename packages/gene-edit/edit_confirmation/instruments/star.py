...
class StarAdapter(Adapter):
    ...
    def run_pta_liquid_handling(self, lysis_ul: float, reaction_ul: float) -> None:
        """The whole-genome amplification liquid handling: lysis add, handoff, reaction-mix add.

        Maps to the validated whole-genome amplification e2e runner. The thermal amplification itself is the
    ...
    def add_reagent(self, label: str, volume_ul: float, tip: str,
                    script: str) -> None:
        """A generic reagent add (whole-genome amplification lysis / reaction / neutralization)."""
        cmd = (f"cd {_STAR_DIR} && ./run_on_pi.sh "
