...
class StarAdapter(Adapter):
    ...
    def run_pta_liquid_handling(self, lysis_ul: float, reaction_ul: float) -> None:
        """The ResolveDNA PTA liquid handling: lysis add, handoff, reaction-mix add.

        Maps to the validated PTA e2e runner. The thermal amplification itself is the
        ODTC's job (a separate action); this is only the deck work up to the handoff.
        """
        cmd = (
            f"cd {_STAR_DIR}/protocols/bio_validation0/pta_wga && "
            f"./run_pta_wga_REAL_DISCARD_TIPS_e2e.sh{self._tipcol()}"
        )
        self._record(
            "run_pta_liquid_handling",
            {"lysis_ul": lysis_ul, "reaction_ul": reaction_ul, "tip_column": self.tip_column},
            resolved_command=cmd,
            note=(f"ResolveDNA PTA: add {lysis_ul} uL lysis, manual lysis handoff, "
                  f"add {reaction_ul} uL WGA reaction mix (TAS-068.5)"),
        )
    ...
    def add_mastermix(self, stage: str, volume_ul: float, tip: str,
        ...
        `script` is the validated ampseq mastermix script this maps to (01 for PCR1,
    ...
    def add_reagent(self, label: str, volume_ul: float, tip: str,
                    script: str) -> None:
        """A generic reagent add (PTA lysis / reaction / neutralization)."""
        cmd = (f"cd {_STAR_DIR} && ./run_on_pi.sh "
               f"protocols/bio_validation0/pta_wga/{script}{self._tipcol()}")
        self._record(
            "add_reagent",
