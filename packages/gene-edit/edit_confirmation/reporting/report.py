...
def _pta_body(data: dict) -> str:
    return (
        "<div class='kv'>"
        f"<div class='k'>Wells</div><div class='v mono'>{_esc(', '.join(data.get('wells', [])))}</div>"
        f"<div class='k'>WGA program</div><div class='v'>{_esc(data.get('wga_program'))}</div>"
        f"<div class='k'>Source</div><div class='v'>{_esc(data.get('wga_source'))}</div>"
        f"<div class='k'>Lysis / reaction</div><div class='v mono'>{_esc(data.get('lysis_ul'))} uL / {_esc(data.get('reaction_ul'))} uL</div>"
        "</div>"
        f"<div class='note'>{_esc(data.get('wga_note',''))}</div>"
    )


def _picogreen_body(data: dict) -> str:
    rows = []
...
def _ampseq_body(data: dict) -> str:
    ...
    return (
        "<div class='kv'>"
        f"<div class='k'>Locus</div><div class='v'>{_esc(data.get('locus'))} ({_esc(data.get('amplicon_bp'))} bp)</div>"
...
def _handoff_body(data: dict) -> str:
    ...
    kv = (
        ...
        f"<div class='k'>Edit / analysis</div><div class='v'>{_esc(data.get('analysis_intent'))}</div>"
        "</div>"
    )
...
<title>Edit-confirmation dossier - {_esc(cfg.run_id)}</title>
<style>{CSS}</style></head>
<body><div class="wrap">
  <div class="eyebrow">Edit-confirmation package</div>
  <h1>{_esc(cfg.run_id)}</h1>
  <p class="sub">PTA + amplicon-seq, QC-gated, for confirming {_esc(cfg.edit_type.value.replace('_',' '))} at {_esc(cfg.locus.name)}.</p>
  <div class="meta">
    <span>Operator <b>{_esc(cfg.operator)}</b></span>
    <span>Mode <b>{_esc(cfg.mode.value)}</b></span>
    <span>Locus <b>{_esc(cfg.locus.name)} ({_esc(cfg.locus.amplicon_bp)} bp)</b></span>
