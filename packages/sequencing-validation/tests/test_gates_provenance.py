...
from sequencing_validation.gates import (
    Comparison,
    Criterion,
    Decision,
    SampleVerdict,
...
from sequencing_validation.provenance import (
    Origin,
    ProvenanceError,
    RunGuard,
    calibrate,
...
def test_range_criterion():
    c = Criterion("c", "metric", Comparison.RANGE, (1.0, 3.0), "", "test")
    assert c.check(2.0) is True
    assert c.check(0.0) is False
    assert c.check(4.0) is False
...
def test_provenance_guard_blocks_calibrate_and_todo():
    guard = RunGuard()
    guard.add(
        transcribed(10.0, "synthetic water fixture", "uL", "motion_volume"),
        tunable(105.0, "standard PCR lid", "C", "lid"),
        calibrate(None, "measure on reader", "uM", "working_conc"),
        todo("confirm workflow source", name="workflow"),
    )
