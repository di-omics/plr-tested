...
through every stage, and runs the fixed sequence:
  ...
  Gate 0 lh_qc -> WGS preparation -> Gate 1 post-WGS preparation -> PCR enrichment -> Gate 2 post-PCR enrichment -> handoff
...
from .stages.pcr_enrichment import PCREnrichment
...
from .stages.wgs_prep import WGSPreparation
from .stages.qc_fluorescent_dsdna import Checkpoint, FluorescentDsDNAQC
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "target": {
                ...
                "target_product_bp": self.config.target.target_product_bp,
            },
            "assay_type": self.config.assay_type.value,
            "method": {
                "profile_kind": self.config.method.profile_kind.value,
                "parameter_source": self.config.method.parameter_source,
            },
...
def build_stages(config: RunConfig) -> List[Stage]:
    """The fixed sequence for sequencing-assay validation."""
    return [
        LiquidHandlingQC(),
        WGSPreparation(),
        FluorescentDsDNAQC(Checkpoint.POST_WGS_PREP),
        PCREnrichment(),
        FluorescentDsDNAQC(Checkpoint.POST_PCR_ENRICHMENT),
        Handoff(),
    ]
