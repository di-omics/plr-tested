...
cli.py - the operator entrypoint.
    ...
    seq-validate run  <manifest> [--out DIR] [--hardware] [--poor-deck]
    seq-validate plan <manifest>
    seq-validate demo [--out DIR]
...
Also runnable as `python -m sequencing_validation ...`.
"""
...
def cmd_plan(args) -> int:
    ...
    print(f"assay_type : {cfg.assay_type.value}")
    print(f"target      : {cfg.target.name}  PCR enrichment product {cfg.target.target_product_bp} bp")
    print(f"deck       : {cfg.deck.name}")
    print(f"method     : {cfg.method.profile_kind.value} ({cfg.method.parameter_source})")
    print(f"pcr1 anneal: {cfg.method.pcr1_anneal_c:g} C")
    print(f"pcr2_cycles: {cfg.method.pcr2_cycles}")
    print("samples    :")
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="seq-validate",
        description="QC-gated WGS preparation and PCR enrichment for sequencing-assay validation",
    )
    p.add_argument("--version", action="version", version=f"seq-validate {__version__}")
