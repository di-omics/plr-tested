...
package has two:
  ...
  compute tier   the simulation and all the QC math. Needs nothing but Python. If this
                 is green, the site can run `seq-validate demo` right now and read a
  ...
  hardware tier  driving the STAR, ODTC, and Tecan. Needs the PyLabRobot fork, the Pi
                 ...
                 `seq-validate doctor --hardware` lists what is still missing.
...
def _compute_checks() -> List[Check]:
    ...
    core_err = _try_import("sequencing_validation.orchestrator")
    checks.append(Check(
...
def _simulation_selftest() -> tuple:
    try:
        ...
        cfg = build_run({
            ...
            "assay": {"type": "generic"},
            "target": {"name": "L", "target_product_bp": 200},
            "samples": [{"id": "a", "well": "A1"}, {"id": "ntc", "well": "H1", "type": "ntc"}],
            "method": {
                "profile_kind": "synthetic_water",
                "parameter_source": "doctor self-test; synthetic water only",
                ...
                "fluorescent_dsdna_standards_ng_per_ml": [0, 100, 200, 300],
            },
            "acceptance": {
                "lh_cv_max_percent": 5,
...
def format_report(checks: List[Check], hardware: bool) -> str:
    ...
    lines.append(f"seq-validate doctor  ({tier} tier)")
    lines.append("")
    for c in checks:
    ...
    if not hardware:
        ...
        if compute_ok:
            lines.append("compute tier ready: run `seq-validate demo`.")
            lines.append("For a real run, check the hardware tier: `seq-validate doctor --hardware`.")
        else:
        ...
        if n_missing == 0:
            ...
            lines.append(f"{n_missing} item(s) to resolve before a hardware run. "
                         "Simulation works regardless: `seq-validate demo`.")
    return "\n".join(lines)
