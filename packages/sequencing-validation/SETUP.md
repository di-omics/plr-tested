...
```bash
seq-validate doctor              # compute tier
seq-validate doctor --hardware   # compute + hardware tier, with the exact fix per gap
```
...
cd plr-tested/packages/sequencing-validation
python3 -m sequencing_validation demo
```
...
```bash
pip install -e .            # gives you the `seq-validate` command and YAML manifests
pip install -e '.[test]'    # to run `pytest`
```

`doctor` (no flag) should be all green after this. If it is, `seq-validate demo` works.
...
against the physical deck, one step at a time. The sequencing validation layout this package
...
### 5. Supply the controlled operator method

Create the manifest's complete `method` and `acceptance` blocks from the site's
approved workflow. Biological liquid volumes, cleanup ratios, thermal programs,
cycle counts, QC dilutions, product volumes, fluorescent dsDNA reader optics and
standards, and acceptance cutoffs are required; the package does not infer them.

Store each biological ODTC profile outside the public repository and reference
its path from the manifest. Validate it with
`05_odtc_run_protocol.py --operator-profile ... --print-xml --dry` before a live
run. The bundled `synthetic_water` manifest is for water-only motion rehearsal.
...
```bash
seq-validate doctor --hardware        # expect all OK
seq-validate run my_run.yaml --hardware
```
