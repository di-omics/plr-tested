...
from pta_targeted_pcr_app.server import create_server
...
class PlanningServerTests(unittest.TestCase):
    ...
    def test_index_explains_protocol_scope_and_keeps_hardware_disabled(self):
        ...
        self.assertEqual(status, 200)
        self.assertIn("authorized WGS/WGA workflow source", html)
        self.assertIn("not the team's targeted PCR SOP", html)
