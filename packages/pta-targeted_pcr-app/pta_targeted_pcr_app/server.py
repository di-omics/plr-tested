...
def app_state() -> Dict[str, object]:
    return {
        "app": {
            "name": "PTA + Targeted PCR bench planner",
            "version": __version__,
            "planning_only": True,
        },
        "constraints": {
...
class PlanningHandler(BaseHTTPRequestHandler):
    ...
    server_version = f"PTATargeted PCRPlanner/{__version__}"
    ...
    def do_GET(self) -> None:
        ...
        if path in ("/", "/index.html"):
            index = resources.files("pta_targeted_pcr_app").joinpath("static").joinpath("index.html")
...
def main(argv: Optional[list] = None) -> int:
    parser = argparse.ArgumentParser(
        prog="pta-targeted_pcr-app",
        description="Planning-only local PTA + Targeted PCR bench wizard.",
    )
    ...
    print(f"PTA + Targeted PCR bench planner: http://127.0.0.1:{port}")
