...
from wgs_pcr_enrichment_app.server import create_server
...
class PlanningServerTests(unittest.TestCase):
    ...
    def test_index_explains_protocol_scope_and_keeps_hardware_disabled(self):
        ...
        self.assertEqual(status, 200)
        self.assertIn("validated WGS preparation specification", html)
        self.assertIn("not the team's PCR enrichment SOP", html)
