...
from .registry import combined_dry_deck, release_summary
...
def app_state() -> Dict[str, object]:
    return {
        "app": {
            "name": "WGS preparation + PCR enrichment bench planner",
            "version": __version__,
            "planning_only": True,
        },
        "constraints": {
...
class PlanningHandler(BaseHTTPRequestHandler):
    ...
    server_version = f"PTAPCR enrichmentPlanner/{__version__}"
    ...
    def do_GET(self) -> None:
        ...
        if path in ("/", "/index.html"):
            index = resources.files("wgs_pcr_enrichment_app").joinpath("static").joinpath("index.html")
...
def main(argv: Optional[list] = None) -> int:
    parser = argparse.ArgumentParser(
        prog="wgs-pcr-enrichment-app",
        description="Planning-only local WGS preparation + PCR enrichment bench wizard.",
    )
    ...
    print(f"WGS preparation + PCR enrichment bench planner: http://127.0.0.1:{port}")
