...
class DeckQuality:
    ...
    which is exactly where single-cell PTA lives, so the model puts the stress there.
    """
...
# A deck good enough to pass at and above a few uL, tight everywhere the ampseq/PTA
...
# ---------------------------------------------------------------------------
# Simulated PicoGreen reads
# ---------------------------------------------------------------------------

@dataclass
class PicoGreenModel:
    """A linear PicoGreen response: signal = blank + slope * concentration(ng/mL)."""
...
PICOGREEN_MODEL = PicoGreenModel()


def simulate_pta_yield_conc_ng_per_ml(run_id: str, sample: Sample) -> float:
    """A simulated post-PTA NEAT dsDNA concentration (ng/mL), by sample type.

    Test and control wells amplify to ResolveDNA-scale yields - tens of ng/uL, i.e. tens
    ...
    """
    rng = det_rng(run_id, "pta_yield", sample.id, sample.well)
    if sample.sample_type is SampleType.NO_TEMPLATE:
        return max(0.0, rng.gauss(80.0, 40.0))         # ~0.08 ng/uL -> fails yield
    center = {
        SampleType.TEST: 25000.0,                      # 25 ng/uL
        SampleType.POSITIVE_CONTROL: 28000.0,
        SampleType.NEGATIVE_CONTROL: 26000.0,
    }.get(sample.sample_type, 25000.0)
